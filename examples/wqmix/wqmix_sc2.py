...
from xuance.paddlepaddle.agents import WQMIX_Agents


def parse_args():
    parser = argparse.ArgumentParser("Example: WQMIX of XuanCe for SMAC environments.")
...
class RunnerSC2:
    def __init__(self, config):
        ...
        self.agents = WQMIX_Agents(config=config, envs=self.envs)  # Create the Independent PPO agents.
        self.config = config
...
if __name__ == "__main__":
    parser = parse_args()
    configs_dict = get_configs(file_dir=f"wqmix_sc2_configs/{parser.env_id}.yaml")
