...
class DCG_utility(nn.Module):
    ...
    Args:
        ...
        device (Optional[Union[str, int, flow.device]]): The device for running the model, default is None.
    """
...
class DCG_payoff(DCG_utility):
    ...
    def forward(self, hidden_states_n: Tensor,
        ...
        """
        input_payoff = flow.stack([flow.cat([hidden_states_n[:, edges_from], hidden_states_n[:, edges_to]], dim=-1),
                                    flow.cat([hidden_states_n[:, edges_to], hidden_states_n[:, edges_from]], dim=-1)],
                                   dim=0)
        ...
        if self.low_rank_payoff:
            ...
            payoffs = flow.matmul(payoffs[:, 0, :].unsqueeze(dim=-1), payoffs[:, 1, :].unsqueeze(
...
class Coordination_Graph(object):
    ...
    def set_coordination_graph(self):
        ...
        self.edges_from = flow.zeros(self.n_edges).long().to(self.device)
        self.edges_to = flow.zeros(self.n_edges).long().to(self.device)
        for i, edge in enumerate(self.edges):
            self.edges_from[i] = edge[0]
            self.edges_to[i] = edge[1]
        self.edges_n_in = oneflow_scatter.scatter_add(src=self.edges_to.new_ones(len(self.edges_to)),
                                                    index=self.edges_to, dim=0, dim_size=self.n_vertexes) \
                          + oneflow_scatter.scatter_add(src=self.edges_to.new_ones(len(self.edges_to)),
