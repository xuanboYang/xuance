...
from xuance.oneflow.policies import BasicQhead, ActorNet, CriticNet, VDN_mixer, QMIX_FF_mixer
...
class BasicQnetwork(Module):
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Tensor = None,
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                q_inputs = flow.concat([outputs['state'], agent_ids], dim=-1)
            else:
...
class MixingQnetwork(BasicQnetwork):
    ...
    def Q_tot(self, individual_values: Dict[str, Tensor], states: Optional[Tensor] = None):
        ...
        if self.use_parameter_sharing:
            ...
            """
            individual_inputs = flow.concat([individual_values[k] for k in self.model_keys],
...
class Qtran_MixingQnetwork(BasicQnetwork):
    ...
    def Q_tran(self, states: Tensor, hidden_states: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        if self.use_parameter_sharing:
            ...
            hidden_states_input = flow.cat([hidden_states[k].unsqueeze(1) for k in self.model_keys], dim=1)
            actions_onehot = flow.cat([one_hot(actions[k].long(), self.n_actions_max).unsqueeze(1)
...
class DCG_policy(Module):
    ...
    def get_hidden_states(self, batch_size: int,
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                hidden_states_n = flow.stack(itemgetter(*self.model_keys)(hidden_states), dim=-2)
...
class MFQnetwork(Module):
    ...
    def forward(self, observation: Tensor, actions_mean: Tensor, agent_ids: Tensor):
        ...
        q_inputs = flow.concat([outputs['state'], actions_mean, agent_ids], dim=-1)
...
class Independent_DDPG_Policy(Module):
    ...
    def Qpolicy(self, observation: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                critic_in = flow.concat([outputs['state'], agent_ids], dim=-1)
            else:
                critic_in = outputs['state']
            q_eval[key] = self.critic[key](flow.concat([critic_in, actions[key]], dim=-1))
    ...
    def Qtarget(self, next_observation: Dict[str, Tensor], next_actions: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                ...
                critic_in = outputs['state']
            q_target[key] = self.target_critic[key](flow.concat([critic_in, next_actions[key]], dim=-1))
        return rnn_hidden_new, q_target
    ...
    def Qpolicy(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                ...
                critic_in = flow.concat([joint_rep_out, agent_ids], dim=-1)
            else:
                if self.use_rnn:
...
class MATD3_Policy(MADDPG_Policy, Module):
    ...
    def Qpolicy(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                ...
                critic_in_A = flow.concat([joint_rep_out_A, agent_ids], dim=-1)
                critic_in_B = flow.concat([joint_rep_out_B, agent_ids], dim=-1)
            else:
                if self.use_rnn:
    ...
    def Qtarget(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        for key in agent_list:
            ...
            q_target[key] = flow.minimum(q_target_A, q_target_B)
