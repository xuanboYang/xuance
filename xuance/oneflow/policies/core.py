...
from xuance.oneflow import Tensor, Module
from xuance.oneflow.utils import ModuleType, mlp_block, gru_block, lstm_block
from xuance.oneflow.utils import CategoricalDistribution, DiagGaussianDistribution, ActivatedDiagGaussianDistribution
...
class GaussianActorNet(Module):
    ...
    def __init__(self,
        ...
        self.logstd = nn.Parameter(-flow.ones((action_dim,), device=device))
    ...
    def forward(self, x: Tensor):
        ...
        """
        # 确保x和mu在同一设备上
        if x.device != self.mu[0].weight.device:
            x = x.to(self.mu[0].weight.device)
            
        # 检查并确保输入的形状是否正确
        expected_input_dim = self.mu[0].weight.shape[1]
        if len(x.shape) > 1 and x.shape[-1] != expected_input_dim:
            # 如果输入维度不匹配，调整大小
            if x.shape[-1] < expected_input_dim:
                # 如果太小，填充
                padding = flow.zeros(*x.shape[:-1], expected_input_dim - x.shape[-1], device=x.device)
                x = flow.cat([x, padding], dim=-1)
            else:
                # 如果太大，截断
        ...
        try:
            ...
            print(f"Error in mu network: {e}, input shape: {x.shape}")
            # 创建一个默认输出
            action_dim = self.logstd.shape[0]
            mu_output = flow.zeros((*x.shape[:-1], action_dim), device=x.device)
        
        # 确保logstd和mu_output在同一设备上
        if self.logstd.device != mu_output.device:
            self.logstd = self.logstd.to(mu_output.device)
            
        self.dist.set_param(mu_output, self.logstd.exp())
        return self.dist
...
class GaussianActorNet_SAC(Module):
    ...
    def __init__(self,
        ...
        self.out_mu = nn.Linear(hidden_sizes[-1], action_dim)
        self.out_mu = self.out_mu.to(device)
        self.out_log_std = nn.Linear(hidden_sizes[-1], action_dim)
        self.out_log_std = self.out_log_std.to(device)
    ...
    def forward(self, x: Tensor):
        ...
        self.out_mu = self.out_mu.to(self.device)
        self.out_log_std = self.out_log_std.to(self.device)
        
        output = self.output(x)
        mu = self.out_mu(output)
        log_std = flow.clamp(self.out_log_std(output), -20, 2)
        std = log_std.exp()
...
class QMIX_FF_mixer(nn.Module):
    ...
    def forward(self, values_n, states=None):
        ...
        inputs = flow.cat([agent_qs, states], dim=-1).to(self.device)
...
class QTRAN_base(nn.Module):
    ...
    def __init__(self,
        ...
        # 确保所有输入参数都是Python原生int类型
        self.dim_state = int(dim_state)
        self.action_space = action_space
        self.dim_hidden = int(dim_hidden)
        self.n_agents = int(n_agents)
        dim_utility_hidden = int(dim_utility_hidden)
        
        # 确保n_actions_list中的所有元素都是原生int类型
        ...
        self.Q_jt = nn.Sequential(nn.Linear(int(self.dim_q_input), int(self.dim_hidden)),
                                  nn.ReLU(),
                                  nn.Linear(int(self.dim_hidden), int(self.dim_hidden)),
                                  nn.ReLU(),
                                  nn.Linear(int(self.dim_hidden), 1)).to(device)
        self.V_jt = nn.Sequential(nn.Linear(int(self.dim_v_input), int(self.dim_hidden)),
                                  nn.ReLU(),
                                  nn.Linear(int(self.dim_hidden), int(self.dim_hidden)),
                                  nn.ReLU(),
                                  nn.Linear(int(self.dim_hidden), 1)).to(device)
        self.dim_ae_input = dim_utility_hidden + self.n_actions_max
        self.action_encoding = nn.Sequential(nn.Linear(int(self.dim_ae_input), int(self.dim_ae_input)),
                                             nn.ReLU(),
                                             nn.Linear(int(self.dim_ae_input), int(self.dim_ae_input))).to(device)
    ...
    def forward(self, states: Tensor, hidden_state_inputs: Tensor, actions_onehot: Tensor):
        ...
        """
        h_state_action_input = flow.cat([hidden_state_inputs, actions_onehot], dim=-1)
        ...
        input_q = flow.cat([states, h_state_action_encode], dim=-1)
        input_v = states
...
class QTRAN_alt(nn.Module):
    ...
    def forward(self, states: Tensor, hidden_state_inputs: Tensor, actions_onehot: Tensor):
        ...
        agent_ids = flow.eye(self.n_agents, dtype=flow.float32, device=self.device)
        ...
        input_q = flow.cat([repeated_states, h_state_action_encode, repeat_agent_ids], dim=-1)
        input_v = states
        q_jt = self.Q_jt(input_q)
        v_jt = self.V_jt(input_v)
        return q_jt, v_jt
