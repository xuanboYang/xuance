...
class NoisyQnetwork(Module):
    ...
    def update_noise(self, noisy_bound: float = 0.0):
        ...
        for parameter in self.eval_Qhead.parameters():
            self.eval_noise_parameter.append(flow.randn_like(parameter) * noisy_bound)
            self.target_noise_parameter.append(flow.randn_like(parameter) * noisy_bound)
...
class C51Qnetwork(Module):
    ...
    def __init__(self,
        ...
        self.supports = flow.nn.Parameter(flow.linspace(self.v_min, self.v_max, self.atom_num),
...
class DDPGPolicy(Module):
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        q_ = self.target_critic(flow.concat([outputs_critic['state'], act], dim=-1))
        return q_[:, 0]
    ...
    def Qaction(self, observation: Union[np.ndarray, dict], action: Tensor):
        ...
        q = self.critic(flow.concat([outputs['state'], action], dim=-1))
        return q[:, 0]
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        q_eval = self.critic(flow.concat([outputs_critic['state'], act], dim=-1))
        return q_eval[:, 0]
...
class TD3Policy(Module):
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        act = self.target_actor(outputs_actor['state'])
        noise = (flow.randn_like(act) * 0.2).clamp(-0.5, 0.5)
        act = (act + noise).clamp(-1, 1)

        qa = self.target_critic_A(flow.concat([outputs_critic_A['state'], act], dim=-1))
        qb = self.target_critic_B(flow.concat([outputs_critic_B['state'], act], dim=-1))
        min_q = flow.min(qa, qb)
        return min_q[:, 0]
    ...
    def Qaction(self, observation: Union[np.ndarray, dict], action: Tensor):
        ...
        q_eval_a = self.critic_A(flow.concat([outputs_critic_A['state'], action], dim=-1))
        q_eval_b = self.critic_B(flow.concat([outputs_critic_B['state'], action], dim=-1))
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        outputs_critic_B = self.critic_B_representation(observation)
        act = self.actor(outputs_actor['state'])
        q_eval_a = self.critic_A(flow.concat([outputs_critic_A['state'], act], dim=-1)).unsqueeze(dim=1)
        q_eval_b = self.critic_B(flow.concat([outputs_critic_B['state'], act], dim=-1)).unsqueeze(dim=1)
...
class PDQNPolicy(Module):
    ...
    def Qtarget(self, state, action):
        input_q = flow.cat((state, action), dim=1)
    ...
    def Qpolicy(self, state):
        ...
        input_q = flow.cat((state, conact), dim=1)
        policy_q = flow.sum(self.qnetwork(input_q))
        return policy_q
...
class MPDQNPolicy(PDQNPolicy):
    ...
    def Qtarget(self, state, action):
        ...
        Q = []
        input_q = flow.cat((state, flow.zeros_like(action)), dim=1)
    ...
    def Qpolicy(self, state):
        ...
        batch_size = state.shape[0]
        Q = []
        input_q = flow.cat((state, flow.zeros_like(conact)), dim=1)
        input_q = input_q.repeat(self.num_disact, 1)
        for i in range(self.num_disact):
            input_q[i * batch_size:(i + 1) * batch_size,
            self.obs_size + self.offsets[i]: self.obs_size + self.offsets[i + 1]] \
...
class SPDQNPolicy(PDQNPolicy):
    ...
    def __init__(self,
        ...
        super(SPDQNPolicy, self).__init__(observation_space, action_space, representation,
                                          ...
                                          normalize, initialize, activation, activation_action, device,
                                          use_distributed_training)
        self.qnetwork = flow.nn.ModuleList()
    ...
    def Qtarget(self, state, action):
        ...
        target_Q = flow.cat(target_Q, dim=1)
        return target_Q
...
class DRQNPolicy(Module):
    ...
    def init_hidden(self, batch):
        hidden_states = flow.zeros(size=(self.recurrent_layer_N, batch, self.rnn_hidden_dim)).to(self.device)
        cell_states = flow.zeros_like(hidden_states).to(self.device) if self.lstm else None
        return hidden_states, cell_states

    def init_hidden_item(self, rnn_hidden, i):
        if self.lstm:
            rnn_hidden[0][:, i] = flow.zeros(size=(self.recurrent_layer_N, self.rnn_hidden_dim)).to(self.device)
            rnn_hidden[1][:, i] = flow.zeros(size=(self.recurrent_layer_N, self.rnn_hidden_dim)).to(self.device)
            return rnn_hidden
        else:
            rnn_hidden[:, i] = flow.zeros(size=(self.recurrent_layer_N, self.rnn_hidden_dim)).to(self.device)
            return rnn_hidden
