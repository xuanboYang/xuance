...
from xuance.oneflow.policies import REGISTRY_Policy, QMIX_mixer
from xuance.oneflow.learners import QMIX_Learner
from xuance.oneflow.agents import MARLAgents
from xuance.oneflow.agents.multi_agent_rl.iql_agents import IQL_Agents
...
class MFQ_Agents(MARLAgents):
    ...
    def __init__(self,
        ...
        optimizer = flow.optim.Adam(policy.parameters(), config.learning_rate, eps=1e-5)
        scheduler = flow.optim.lr_scheduler.LinearLR(optimizer, start_factor=1.0, end_factor=self.end_factor_lr_decay,
    ...
    def act(self, obs_n, *rnn_hidden, test_mode=False, act_mean=None, agent_mask=None, avail_actions=None):
        ...
        agents_id = flow.eye(self.n_agents).unsqueeze(0).expand(batch_size, -1, -1).to(self.device)
        obs_in = flow.Tensor(obs_n).to(self.device)
        act_mean = flow.Tensor(act_mean).unsqueeze(dim=-2).repeat(1, self.n_agents, 1).to(self.device)
        ...
        n_alive = flow.Tensor(agent_mask).sum(dim=-1).unsqueeze(-1).repeat(1, self.dim_act).to(self.device)
        action_n_mask = flow.Tensor(agent_mask).unsqueeze(-1).repeat(1, 1, self.dim_act).to(self.device)
