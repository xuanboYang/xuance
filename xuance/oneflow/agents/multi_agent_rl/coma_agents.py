...
class COMA_Agents(OnPolicyMARLAgents):
    ...
    def action(self,
        ...
        # 确保输入在正确的设备上
        if agents_id is not None and agents_id.device != device:
            agents_id = agents_id.to(device)
        
        # 确保RNN隐藏状态在正确的设备上
        if self.use_rnn and rnn_hidden_actor is not None:
            for key in rnn_hidden_actor:
                if rnn_hidden_actor[key][0].device != device:
                    rnn_hidden_actor[key] = (rnn_hidden_actor[key][0].to(device), rnn_hidden_actor[key][1].to(device))
        
        # 确保每个观测在正确的设备上
        for key in obs_input:
            if isinstance(obs_input[key], Tensor) and obs_input[key].device != device:
                obs_input[key] = obs_input[key].to(device)
            
        # 确保可用动作掩码在正确的设备上
        if avail_actions_input is not None:
            for key in avail_actions_input:
                if isinstance(avail_actions_input[key], Tensor) and avail_actions_input[key].device != device:
                    avail_actions_input[key] = avail_actions_input[key].to(device)
        ...
        if self.use_parameter_sharing:
            ...
            # 计算动作的对数概率
            log_pi_a = pi_dists[key].log_prob(actions_sample)
            log_pi_a_dict = {k: log_pi_a.cpu().detach().numpy().reshape(n_env, self.n_agents)[:, i] 
                           for i, k in enumerate(self.agent_keys)}
        else:
            agents_id = flow.eye(self.n_agents).unsqueeze(0).expand(n_env, -1, -1).to(device)
            ...
            actions_out = flow.stack(itemgetter(*self.agent_keys)(actions_sample), dim=-1)
            ...
            # 计算每个智能体动作的对数概率
            log_pi_a_dict = {k: pi_dists[k].log_prob(actions_sample[k]).cpu().detach().numpy() for k in self.agent_keys}
        ...
        if not test_mode:  # calculate target values
            ...
            # 确保state在正确的设备上
            state_tensor = Tensor(state).to(device)
            
            # 确保actions_onehot在正确的设备上
            for key in actions_onehot:
                if actions_onehot[key].device != device:
                    actions_onehot[key] = actions_onehot[key].to(device)
                
            # 确保critic的RNN隐藏状态在正确的设备上
            if self.use_rnn and rnn_hidden_critic is not None:
                for key in rnn_hidden_critic:
                    if rnn_hidden_critic[key][0].device != device:
                        rnn_hidden_critic[key] = (rnn_hidden_critic[key][0].to(device), rnn_hidden_critic[key][1].to(device))

            rnn_hidden_critic_new, values_out = self.policy.get_values(state=state_tensor,
        ...
        return {"rnn_hidden_actor": rnn_hidden_actor_new, "rnn_hidden_critic": rnn_hidden_critic_new,
                "actions": actions_dict, "log_pi_old": log_pi_a_dict, "values": values_dict}
    ...
    def values_next(self,
        ...
        agents_id = flow.eye(self.n_agents).unsqueeze(0).repeat(n_env, 1, 1).to(device)
        if self.use_rnn:
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                # 确保RNN隐藏状态在正确的设备上
                if rnn_hidden_critic_i[key][0].device != device:
                    rnn_hidden_critic_i[key] = (rnn_hidden_critic_i[key][0].to(device), rnn_hidden_critic_i[key][1].to(device))
                obs_array = np.array(itemgetter(*self.agent_keys)(obs_dict))
                obs_input = {key: obs_array.reshape([bs, 1, -1])}
                actions_tensor = actions_tensor.reshape(n_env, 1, self.n_agents).to(device)
            else:
                obs_input = {key: np.array([itemgetter(*self.agent_keys)(obs_dict)])}
                actions_tensor = actions_tensor.reshape(n_env, self.n_agents).to(device)
            # 确保obs_input在正确的设备上
            if isinstance(obs_input[key], Tensor) and obs_input[key].device != device:
                obs_input[key] = obs_input[key].to(device)
            ...
            if self.use_rnn:
                ...
                # 确保RNN隐藏状态在正确的设备上
                for key in rnn_hidden_critic_i:
                    if rnn_hidden_critic_i[key][0].device != device:
                        rnn_hidden_critic_i[key] = (rnn_hidden_critic_i[key][0].to(device), rnn_hidden_critic_i[key][1].to(device))
    ...
    def train(self, n_steps):
        ...
        for _ in tqdm(range(n_steps)):
            ...
            actions_dict, log_pi_a_dict = policy_out['actions'], policy_out['log_pi_old']
