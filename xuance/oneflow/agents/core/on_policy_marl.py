...
from xuance.oneflow.agents.base import MARLAgents
...
class OnPolicyMARLAgents(MARLAgents):
    ...
    def store_experience(self, obs_dict, avail_actions, actions_dict, log_pi_a, rewards_dict, values_dict,
        ...
        # 确保state字段被添加，即使use_global_state为False
        # 对于MAPPO算法，我们需要state字段，无论use_global_state是否为True
        if 'state' in kwargs and kwargs['state'] is not None:
            experience_data['state'] = np.array(kwargs['state'])
        else:
            # 如果没有提供state，则从obs创建一个合成的state
            # 将所有智能体的观察连接起来作为全局状态
            all_obs = []
            for k in self.agent_keys:
                agent_obs = np.array([data[k] for data in obs_dict])
                all_obs.append(agent_obs)
            # 沿着第二个维度（特征维度）连接
            # 形状从 [n_envs, dim_obs] 变为 [n_envs, n_agents*dim_obs]
            combined_state = np.concatenate(all_obs, axis=1)
            experience_data['state'] = combined_state
    ...
    def values_next(self,
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                agents_id = flow.eye(self.n_agents).unsqueeze(0).expand(n_env, -1, -1).reshape(batch_size, 1, -1).to(
                    self.device)
            else:
                obs_input = {key: np.array([itemgetter(*self.agent_keys)(obs_dict)])}
                agents_id = flow.eye(self.n_agents).unsqueeze(0).expand(n_env, -1, -1).to(self.device)
