...
class SACDIS_Learner(Learner):
    ...
    def update(self, **samples):
        ...
        act_batch = flow.as_tensor(samples['actions'], device=self.device).unsqueeze(-1)
        next_batch = flow.as_tensor(samples['obs_next'], device=self.device)
        rew_batch = flow.as_tensor(samples['rewards'], device=self.device).unsqueeze(-1)
        ter_batch = flow.as_tensor(samples['terminals'], dtype=flow.float, device=self.device).reshape([-1, 1])

        # actor update
        action_prob, log_pi, policy_q_1, policy_q_2 = self.policy.Qpolicy(obs_batch)
        policy_q = flow.min(policy_q_1, policy_q_2)
