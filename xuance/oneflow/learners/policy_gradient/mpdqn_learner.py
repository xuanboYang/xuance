...
class MPDQN_Learner(Learner):
    def __init__(self,
        ...
        conactor_optimizer = flow.optim.Adam(self.policy.conactor.parameters(), self.config.learning_rate)
        qnetwork_optimizer = flow.optim.Adam(self.policy.qnetwork.parameters(), self.config.learning_rate)
        self.optimizers = [conactor_optimizer, qnetwork_optimizer]
        conactor_lr_scheduler = flow.optim.lr_scheduler.LinearLR(conactor_optimizer,
                                                                  start_factor=1.0,
                                                                  end_factor=self.end_factor_lr_decay,
                                                                  total_iters=self.config.running_steps)
        qnetwork_lr_scheduler = flow.optim.lr_scheduler.LinearLR(qnetwork_optimizer,
                                                                  start_factor=1.0,
                                                                  end_factor=self.end_factor_lr_decay,
                                                                  total_iters=self.config.running_steps)
    ...
    def update(self, **samples):
        ...
        hyact_batch = flow.as_tensor(samples['actions'], device=self.device)
        ...
        with flow.no_grad():
            ...
            target_q = flow.max(target_q, 1, keepdim=True)[0].squeeze()
