...
class PPOCLIP_Learner(Learner):
    ...
    def update(self, **samples):
        ...
        ret_batch = flow.as_tensor(samples['returns'], device=self.device)
        adv_batch = flow.as_tensor(samples['advantages'], device=self.device)
        old_logp_batch = flow.as_tensor(samples['aux_batch']['old_logp'], device=self.device)
        ...
        a_loss = -flow.minimum(surrogate1, surrogate2).mean()
