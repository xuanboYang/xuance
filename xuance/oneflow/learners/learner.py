...
from xuance.oneflow import Tensor
...
class Learner(ABC):
    def __init__(self,
        ...
        self.policy = policy
        self.optimizer: Union[dict, list, Optional[flow.optim.Optimizer]] = None
        self.scheduler: Union[dict, list, Optional[flow.optim.lr_scheduler.LinearLR]] = None
    ...
    def save_model(self, model_path):
        flow.save(self.policy.state_dict(), model_path)
        if self.distributed_training:
            self.save_snapshot()
    ...
    def load_model(self, path, model=None):
        ...
        self.policy.load_state_dict(flow.load(str(model_path), map_location={
    ...
    def load_snapshot(self, snapshot_path):
        ...
        snapshot = flow.load(snapshot_path, map_location=loc)
    ...
    def save_snapshot(self):
        ...
        flow.save(snapshot, snapshot_pt)
...
class LearnerMAS(ABC):
    ...
    def load_model(self, path, model=None):
        if not os.path.exists(path):
            print(f"警告：模型目录 '{path}' 不存在，将创建新目录并初始化模型。")
        ...
        if model is not None:
            path = os.path.join(path, model)
            if not os.path.exists(path):
                print(f"警告：模型文件 '{model}' 不存在，将初始化新模型。")
                return
        else:
            seed_files = [f for f in file_names if "seed_" in f]
            if len(seed_files) == 0:
                print(f"警告：在 '{path}' 中没有找到种子目录，将初始化新模型。")
                return
                
            seed_files.sort()
            path = os.path.join(path, seed_files[-1])
            if not os.path.exists(path):
                print(f"警告：种子目录 '{path}' 不存在，将初始化新模型。")
                return
        ...
        if os.path.exists(path + "/obs_rms.npy"):
            model_names.remove("obs_rms.npy")
        if len(model_names) == 0:
            print(f"警告：在 '{path}' 中没有找到模型文件，将初始化新模型。")
            return
            
        model_names.sort()
        model_path = os.path.join(path, model_names[-1])
        try:
            # 简化map_location的使用，直接指定设备
            self.policy.load_state_dict(flow.load(str(model_path), map_location=self.device))
            print(f"成功从 '{path}' 加载模型。")
