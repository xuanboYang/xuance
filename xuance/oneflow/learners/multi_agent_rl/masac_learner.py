...
class MASAC_Learner(ISAC_Learner):
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            if self.use_parameter_sharing:
                ...
                actions_eval_joint = flow.concat(itemgetter(*self.model_keys)(actions_eval_detach_others),
            ...
            policy_q = flow.min(policy_q_1[key], policy_q_2[key]).reshape(bs)
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            actions_eval_joint = flow.concat(itemgetter(*self.agent_keys)(actions_eval),
        ...
        for key in self.model_keys:
            ...
            policy_q = flow.min(policy_q_1[key], policy_q_2[key]).reshape(bs_rnn, seq_len)
            loss_a = ((self.alpha[key] * log_pi_eval_i - policy_q) * mask_values).sum() / mask_values.sum()
            self.optimizer[key]['actor'].zero_grad()
            loss_a.backward()
            if self.use_grad_clip:
