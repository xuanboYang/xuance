...
class QTRAN_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.optimizer = flow.optim.Adam(self.policy.parameters_model, config.learning_rate, eps=1e-5)
    ...
    def update(self, sample):
        ...
        if self.config.agent == "QTRAN_base":
            ...
            loss_opt = flow.mean(error_opt ** 2)  # Opt loss
            ...
            loss_nopt = flow.mean(error_nopt ** 2)  # NOPT loss
            ...
            agent_mask_diag = (1 - flow.eye(self.n_agents, dtype=flow.float32,
            ...
            error_nopt_min = flow.min(error_nopt, dim=-1).values
            loss_nopt = flow.mean(error_nopt_min ** 2)  # NOPT loss
    ...
    def update_rnn(self, sample):
        ...
        if self.config.agent == "QTRAN_base":
            ...
            v_joint_repeated = v_joint.repeat(1, self.n_agents).view(-1, 1)
            error_nopt = q_eval_count + q_sum_mask.view(-1, 1) - q_count_for_nopt.detach() + v_joint_repeated
            error_nopt_min = flow.min(error_nopt, dim=-1).values * filled_n.reshape(-1)
