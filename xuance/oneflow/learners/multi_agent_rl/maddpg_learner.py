...
class MADDPG_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.optimizer = {
            key: {'actor': flow.optim.Adam(self.policy.parameters_actor[key], self.config.learning_rate_actor, eps=1e-5),
                  'critic': flow.optim.Adam(self.policy.parameters_critic[key], self.config.learning_rate_critic, eps=1e-5)}
            for key in self.model_keys}
        self.scheduler = {
            key: {'actor': flow.optim.lr_scheduler.LinearLR(self.optimizer[key]['actor'],
                                                             start_factor=1.0,
                                                             end_factor=self.end_factor_lr_decay,
                                                             total_iters=self.config.running_steps),
                  'critic': flow.optim.lr_scheduler.LinearLR(self.optimizer[key]['critic'],
                                                              start_factor=1.0,
                                                              end_factor=self.end_factor_lr_decay,
                                                              total_iters=self.config.running_steps)}
            for key in self.model_keys}
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            bs = batch_size
            obs_joint = flow.concat(itemgetter(*self.agent_keys)(obs), dim=-1).reshape(batch_size, -1)
            next_obs_joint = flow.concat(itemgetter(*self.agent_keys)(obs_next), dim=-1).reshape(batch_size, -1)
            actions_joint = flow.concat(itemgetter(*self.agent_keys)(actions), dim=-1).reshape(batch_size, -1)
        ...
        if self.use_parameter_sharing:
            ...
            actions_next_joint = flow.concat(itemgetter(*self.model_keys)(actions_next), -1).reshape(batch_size, -1)
        ...
        for key in self.model_keys:
            ...
            if self.use_grad_clip:
                flow.nn.utils.clip_grad_norm_(self.policy.parameters_critic[key], self.grad_clip_norm)
            ...
            if self.use_parameter_sharing:
                ...
                act_eval = flow.concat(itemgetter(*self.agent_keys)(a_joint), dim=-1).reshape(batch_size, -1)
            ...
            loss_a.backward()
            if self.use_grad_clip:
                flow.nn.utils.clip_grad_norm_(self.policy.parameters_actor[key], self.grad_clip_norm)
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            obs_joint = flow.concat(itemgetter(*self.agent_keys)(obs), dim=-1).reshape(batch_size, seq_len + 1, -1)
            actions_joint = flow.concat(itemgetter(*self.agent_keys)(actions), dim=-1).reshape(batch_size, seq_len, -1)
        ...
        if self.use_parameter_sharing:
            ...
            actions_next_joint = flow.concat(itemgetter(*self.agent_keys)(actions_next),
        ...
        for key in self.model_keys:
            ...
            if self.use_parameter_sharing:
                ...
                act_eval = flow.concat(itemgetter(*self.agent_keys)(a_dict), dim=-1).reshape(batch_size, seq_len, -1)
