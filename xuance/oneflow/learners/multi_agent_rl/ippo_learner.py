...
class IPPO_Learner(IAC_Learner):
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            ratio = flow.exp(log_pi - log_pi_old[key]).reshape(bs)
    ...
    def update_rnn(self, sample):
        ...
        for key in self.model_keys:
            ...
            ratio = flow.exp(log_pi - log_pi_old[key])
            surrogate1 = ratio * advantages[key]
            surrogate2 = flow.clip(ratio, 1 - self.clip_range, 1 + self.clip_range) * advantages[key]
            loss_a.append(-(flow.min(surrogate1, surrogate2) * mask_values).sum() / mask_values.sum())
