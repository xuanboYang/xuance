...
class IQL_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.optimizer = {key: flow.optim.Adam(self.policy.parameters_model[key], config.learning_rate, eps=1e-5)
                          for key in self.model_keys}
        self.scheduler = {key: flow.optim.lr_scheduler.LinearLR(self.optimizer[key],
                                                                 start_factor=1.0,
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            if self.use_grad_clip:
                flow.nn.utils.clip_grad_norm_(self.policy.parameters_model[key], self.grad_clip_norm)
