...
class MFQ_Learner(LearnerMAS):
    def __init__(self,
                 ...
                 scheduler: Optional[flow.optim.lr_scheduler._LRScheduler] = None,
                 device: Optional[Union[int, str, flow.device]] = None,
                 model_dir: str = "./",
                 gamma: float = 0.99,
    ...
    def update(self, sample):
        self.iterations += 1
        obs = flow.Tensor(sample['obs']).to(self.device)
        actions = flow.Tensor(sample['actions']).to(self.device)
        obs_next = flow.Tensor(sample['obs_next']).to(self.device)
        act_mean = flow.Tensor(sample['act_mean']).to(self.device)
        act_mean_next = flow.Tensor(sample['act_mean_next']).to(self.device)
        rewards = flow.Tensor(sample['rewards']).to(self.device)
        terminals = flow.Tensor(sample['terminals']).float().reshape(-1, self.n_agents, 1).to(self.device)
        agent_mask = flow.Tensor(sample['agent_mask']).float().reshape(-1, self.n_agents, 1).to(self.device)
        IDs = flow.eye(self.n_agents).unsqueeze(0).expand(self.args.batch_size, -1, -1).to(self.device)
        ...
        v_mf = flow.bmm(q_next.reshape(-1, 1, shape[-1]), pi.unsqueeze(-1).reshape(-1, shape[-1], 1))
