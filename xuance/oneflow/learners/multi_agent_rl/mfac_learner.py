...
class MFAC_Learner(LearnerMAS):
    ...
    def update(self, sample):
        info = {}
        self.iterations += 1
        state = flow.Tensor(sample['state']).to(self.device)
        ...
        returns = flow.Tensor(sample['returns']).to(self.device)
        agent_mask = flow.Tensor(sample['agent_mask']).float().reshape(-1, self.n_agents, 1).to(self.device)
        batch_size = obs.shape[0]
        IDs = flow.eye(self.n_agents).unsqueeze(0).expand(batch_size, -1, -1).to(self.device)
        ...
        if self.use_grad_clip:
            grad_norm = flow.nn.utils.clip_grad_norm_(self.policy.parameters(), self.grad_clip_norm)
