...
class IAC_Learner(LearnerMAS):
    ...
    def build_optimizer(self):
        self.optimizer = flow.optim.Adam(self.policy.parameters_model, lr=self.learning_rate, eps=1e-5,
    ...
    def build_training_data(self, sample: Optional[dict],
        ...
        """
        Build training data from experience buffer for multiple different agents.
        ...
        if use_parameter_sharing:
            ...
            for agent_id in self.model_keys:
                ...
                # 检查 'advantages' 键是否存在
                if 'advantages' in sample:
                    advantages[agent_id] = Tensor(sample['advantages'][agent_id]).to(self.device)
                else:
                    # 如果不存在，则使用 returns - values 计算
                    advantages[agent_id] = returns[agent_id] - values[agent_id]
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            if self.use_value_clip:
                ...
                loss_c_ = flow.max(loss_v, loss_v_clipped) * mask_values
        ...
        if self.use_grad_clip:
            grad_norm = flow.nn.utils.clip_grad_norm_(self.policy.parameters_model, self.grad_clip_norm)
