...
class ISAC_Learner(LearnerMAS):
    def __init__(self,
        ...
        if self.use_automatic_entropy_tuning:
            ...
            self.log_alpha = {key: nn.Parameter(flow.zeros(1, requires_grad=True, device=self.device))
                              for key in self.model_keys}
            self.alpha = {key: self.log_alpha[key].exp() for key in self.model_keys}
            self.alpha_optimizer = {key: flow.optim.Adam([self.log_alpha[key]], lr=config.learning_rate_actor)
                                    for key in self.model_keys}
    ...
    def update_rnn(self, sample):
        ...
        for key in self.model_keys:
            ...
            policy_q = flow.min(policy_q_1[key][:, :-1], policy_q_2[key][:, :-1]).reshape(bs_rnn, seq_len)
            loss_a = ((self.alpha[key] * log_pi_eval_i - policy_q) * mask_values).sum() / mask_values.sum()
            self.optimizer[key]['actor'].zero_grad()
            loss_a.backward()
            if self.use_grad_clip:
