...
def get_runner(method,
    ...
    if dl_toolbox == "paddle":
        rank = int(os.environ['RANK']) if distributed_training else 0
        from xuance.paddlepaddle.runners import REGISTRY_Runner
        if rank == 0:
            print("Deep learning toolbox: PaddlePaddle.")

    elif dl_toolbox == "torch":
        rank = int(os.environ['RANK']) if distributed_training else 0
        from xuance.oneflow.runners import REGISTRY_Runner
        if rank == 0:
            print("Deep learning toolbox: PyTorch.")
            
    elif dl_toolbox == "oneflow":
        rank = int(os.environ['RANK']) if distributed_training else 0
        from xuance.oneflow.runners import REGISTRY_Runner
        if rank == 0:
            print("Deep learning toolbox: OneFlow.")
