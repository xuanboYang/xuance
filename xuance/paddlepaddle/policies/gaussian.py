...
class PPGActorCritic(Layer):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            self.actor = paddle.DataParallel(self.actor)
            self.critic = paddle.DataParallel(self.critic)
            self.aux_critic = paddle.DataParallel(self.aux_critic)
...
class SACPolicy(Layer):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.critic_1_representation._get_name() != "Basic_Identical":
                self.critic_1_representation = paddle.DataParallel(self.critic_1_representation)
            if self.critic_2_representation._get_name() != "Basic_Identical":
                self.critic_2_representation = paddle.DataParallel(self.critic_2_representation)
            self.actor = paddle.DataParallel(self.actor)
            self.critic_1 = paddle.DataParallel(self.critic_1)
            self.critic_2 = paddle.DataParallel(self.critic_2)
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        q_1 = self.critic_1(paddle.concat([outputs_critic_1['state'], act_sample], axis=-1))
        q_2 = self.critic_2(paddle.concat([outputs_critic_2['state'], act_sample], axis=-1))
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        target_q_1 = self.target_critic_1(paddle.concat([outputs_critic_1['state'], new_act_sample], axis=-1))
        target_q_2 = self.target_critic_2(paddle.concat([outputs_critic_2['state'], new_act_sample], axis=-1))
        target_q = paddle.min(target_q_1, target_q_2)
    ...
    def Qaction(self, observation: Union[np.ndarray, dict], action: Tensor):
        ...
        outputs_critic_1 = self.critic_1_representation(observation)
        outputs_critic_2 = self.critic_2_representation(observation)
        q_1 = self.critic_1(paddle.concat([outputs_critic_1['state'], action], axis=-1))
        q_2 = self.critic_2(paddle.concat([outputs_critic_2['state'], action], axis=-1))
        return q_1[:, 0], q_2[:, 0]
