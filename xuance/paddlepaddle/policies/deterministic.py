...
from xuance.paddlepaddle import Layer, Tensor, DataParallel
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.representation._get_name() != "Basic_Identical":
                self.representation = paddle.DataParallel(self.representation)
            self.eval_Qhead = paddle.DataParallel(self.eval_Qhead)
...
class NoisyQnetwork(Layer):
    ...
    def update_noise(self, noisy_bound: float = 0.0):
        ...
        for parameter in self.eval_Qhead.parameters():
            self.eval_noise_parameter.append(paddle.randn(parameter.shape) * noisy_bound)
            self.target_noise_parameter.append(paddle.randn(parameter.shape) * noisy_bound)
...
class C51Qnetwork(Layer):
    ...
    def __init__(self,
        ...
        # PaddlePaddle 等效代码
        self.supports = paddle.create_parameter(
            shape=[self.atom_num],  # 参数形状
            dtype='float32',  # 数据类型
            attr=paddle.ParamAttr(  # 参数属性
                trainable=False  # 设置为不可训练
            ),
            default_initializer=paddle.nn.initializer.Assign(  # 使用 linspace 初始化
                paddle.linspace(self.v_min, self.v_max, self.atom_num)
            )
        )
        ...
        if self.distributed_training:
            ...
            if self.representation._get_name() != "Basic_Identical":
                self.representation = paddle.DataParallel(self.representation)
            self.eval_Zhead = paddle.DataParallel(self.eval_Zhead)
...
class DDPGPolicy(Layer):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.actor_representation._get_name() != "Basic_Identical":
                self.actor_representation = paddle.DataParallel(self.actor_representation)
            if self.critic_representation._get_name() != "Basic_Identical":
                self.critic_representation = paddle.DataParallel(self.critic_representation)
            self.actor = paddle.DataParallel(self.actor)
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        q_ = self.target_critic(paddle.concat([outputs_critic['state'], act], axis=-1))
        return q_[:, 0]
    ...
    def Qaction(self, observation: Union[np.ndarray, dict], action: Tensor):
        ...
        q = self.critic(paddle.concat([outputs['state'], action], axis=-1))
        return q[:, 0]
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        q_eval = self.critic(paddle.concat([outputs_critic['state'], act], axis=-1))
        return q_eval[:, 0]
...
class TD3Policy(Layer):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.critic_A_representation._get_name() != "Basic_Identical":
                self.critic_A_representation = paddle.DataParallel(self.critic_A_representation)
            if self.critic_B_representation._get_name() != "Basic_Identical":
                self.critic_B_representation = paddle.DataParallel(self.critic_B_representation)
            self.actor = paddle.DataParallel(self.actor)
            self.critic_A = paddle.DataParallel(self.critic_A)
            self.critic_B = paddle.DataParallel(self.critic_B)
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        noise = (paddle.randn(shape=act.shape, dtype=act.dtype) * 0.2).clip(-0.5, 0.5)
        act = (act + noise).clamp(-1, 1)

        qa = self.target_critic_A(paddle.concat([outputs_critic_A['state'], act], axis=-1))
        qb = self.target_critic_B(paddle.concat([outputs_critic_B['state'], act], axis=-1))
        min_q = paddle.min(qa, qb)
        return min_q[:, 0]
    ...
    def Qaction(self, observation: Union[np.ndarray, dict], action: Tensor):
        ...
        q_eval_a = self.critic_A(paddle.concat([outputs_critic_A['state'], action], axis=-1))
        q_eval_b = self.critic_B(paddle.concat([outputs_critic_B['state'], action], axis=-1))
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        outputs_critic_B = self.critic_B_representation(observation)
        act = self.actor(outputs_actor['state'])
        q_eval_a = self.critic_A(paddle.concat([outputs_critic_A['state'], act], axis=-1)).unsqueeze(axis=1)
        q_eval_b = self.critic_B(paddle.concat([outputs_critic_B['state'], act], axis=-1)).unsqueeze(axis=1)
...
class PDQNPolicy(Layer):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.representation._get_name() != "Basic_Identical":
                self.representation = paddle.DataParallel(self.representation)
            self.qnetwork = paddle.DataParallel(self.qnetwork)
            self.conactor = paddle.DataParallel(self.conactor)
    ...
    def Qtarget(self, state, action):
        input_q = paddle.concat((state, action), axis=1)
    ...
    def Qpolicy(self, state):
        ...
        input_q = paddle.concat((state, conact), axis=1)
        policy_q = paddle.sum(self.qnetwork(input_q))
        return policy_q
...
class MPDQNPolicy(PDQNPolicy):
    ...
    def Qtarget(self, state, action):
        ...
        input_q = paddle.concat([state, paddle.zeros_like(action)], axis=1)
    ...
    def Qpolicy(self, state):
        ...
        batch_size = state.shape[0]
        Q = []
        input_q = paddle.concat((state, paddle.zeros_like(conact)), axis=1)
        input_q = input_q.repeat(self.num_disact, 1)
        for i in range(self.num_disact):
            input_q[i * batch_size:(i + 1) * batch_size,
            self.obs_size + self.offsets[i]: self.obs_size + self.offsets[i + 1]] \
...
class SPDQNPolicy(PDQNPolicy):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            dist.init_parallel_env()
            for k in range(self.num_disact):
                self.qnetwork[k] = paddle.DataParallel(self.qnetwork[k])
            self.conactor = paddle.DataParallel(self.conactor)
    ...
    def Qtarget(self, state, action):
        ...
        target_Q = paddle.concat(target_Q, axis=1)
        return target_Q
...
class DRQNPolicy(Layer):
    ...
    def init_hidden(self, batch):
        ...
        hidden_states = paddle.zeros(
            shape=(self.recurrent_layer_N, batch, self.rnn_hidden_dim),
            dtype='float32'
        ).cuda() if self.device == 'gpu' else paddle.zeros(
            shape=(self.recurrent_layer_N, batch, self.rnn_hidden_dim),
            dtype='float32'
        )

        cell_states = paddle.zeros_like(hidden_states).cuda() if self.lstm and self.device == 'gpu' else (
            paddle.zeros_like(hidden_states) if self.lstm else None
        )
    ...
    def init_hidden_item(self, rnn_hidden, i):
        if self.lstm:
            ...
            # 创建零张量并赋值到 rnn_hidden 的对应位置
            rnn_hidden[0][:, i] = paddle.zeros(shape=(self.recurrent_layer_N, self.rnn_hidden_dim), dtype='float32')
            if self.device == 'gpu':
                rnn_hidden[0][:, i] = rnn_hidden[0][:, i].cuda()

            rnn_hidden[1][:, i] = paddle.zeros(shape=(self.recurrent_layer_N, self.rnn_hidden_dim), dtype='float32')
            if self.device == 'gpu':
                rnn_hidden[1][:, i] = rnn_hidden[1][:, i].cuda()

            return rnn_hidden
        else:
            # rnn_hidden[:, i] = torch.zeros(size=(self.recurrent_layer_N, self.rnn_hidden_dim)).to(self.device)
            rnn_hidden[:, i] = paddle.zeros(shape=(self.recurrent_layer_N, self.rnn_hidden_dim), dtype='float32')
            if self.device == 'gpu':
                rnn_hidden[:, i] = rnn_hidden[:, i].cuda()
