...
from xuance.paddlepaddle import Tensor, Layer
from xuance.paddlepaddle.utils import ModuleType, mlp_block, gru_block, lstm_block
from xuance.paddlepaddle.utils import CategoricalDistribution, DiagGaussianDistribution, ActivatedDiagGaussianDistribution


class BasicQhead(Layer):
    """
...
class C51Qhead(Layer):
    ...
    def forward(self, x: Tensor):
        ...
        dist_probs = F.softmax(dist_logits, axis=-1)
        return dist_probs


class QRDQNhead(Layer):
    """
...
class CategoricalActorNet(Layer):
    """
...
class GaussianActorNet(Layer):
    ...
    def __init__(self,
        ...
        self.logstd = self.create_parameter(
            shape=(action_dim,),
            default_initializer=paddle.nn.initializer.Constant(value=-1.0)
        )
        if device == 'gpu':
            self.logstd = self.logstd.cuda()
...
class GaussianActorNet_SAC(Layer):
    """
    ...
    def forward(self, x: Tensor):
        ...
        log_std = paddle.clip(self.out_log_std(output), min=-20, max=2)
        std = log_std.exp()
        self.dist.set_param(mu, std)
        return self.dist


class VDN_mixer(nn.Layer):
    """
    ...
    def forward(self, values_n, states=None):
        return values_n.sum(axis=1)


class QMIX_mixer(nn.Layer):
    """
    ...
    def forward(self, values_n, states):
        ...
        """
        states = paddle.to_tensor(states, dtype=paddle.float32)
        # states = states.reshape(-1, self.dim_state)
        states = paddle.reshape(states, shape=[-1, self.dim_state])
        ...
        w_1 = paddle.abs(self.hyper_w_1(states))  # 计算绝对值
        w_1 = paddle.reshape(w_1, shape=[-1, self.n_agents, self.dim_hidden])  # 改变形状
        b_1 = self.hyper_b_1(states)
        b_1 = paddle.reshape(b_1, shape=[-1, 1, self.dim_hidden])  # 改变形状
        hidden = paddle.nn.functional.elu(paddle.matmul(agent_qs, w_1) + b_1)  # 计算隐藏层输出
        ...
        # b_2 = b_2.view(-1, 1, 1)
        w_2 = paddle.abs(self.hyper_w_2(states))  # 计算绝对值
        w_2 = paddle.reshape(w_2, shape=[-1, self.dim_hidden, 1])  # 改变形状
        b_2 = self.hyper_b_2(states)
        b_2 = paddle.reshape(b_2, shape=[-1, 1, 1])  # 改变形状

        # Compute final output
        # y = torch.bmm(hidden, w_2) + b_2
        y = paddle.matmul(hidden, w_2) + b_2  # 矩阵乘法加偏置

        # Reshape and return
        # q_tot = y.view(-1, 1)
        q_tot = paddle.reshape(y, shape=[-1, 1])
        return q_tot


class QMIX_FF_mixer(nn.Layer):
    """
    ...
    def forward(self, values_n, states=None):
        ...
        states = paddle.reshape(states, shape=[-1, self.dim_state])
        agent_qs = paddle.reshape(values_n, shape=[-1, self.n_agents])
        inputs = paddle.concat([agent_qs, states], axis=-1)
        if self.device == 'gpu':
            inputs = inputs.cuda()
...
class QTRAN_base(nn.Layer):
    ...
    def forward(self, states: Tensor, hidden_state_inputs: Tensor, actions_onehot: Tensor):
        ...
        # 拼接 hidden_state_inputs 和 actions_onehot
        h_state_action_input = paddle.concat([hidden_state_inputs, actions_onehot], axis=-1)

        # 编码并调整形状
        h_state_action_encode = self.action_encoding(h_state_action_input)
        h_state_action_encode = paddle.reshape(h_state_action_encode, shape=[-1, self.n_agents, self.dim_ae_input])

        # 按照 agent 维度求和
        h_state_action_encode = h_state_action_encode.sum(axis=1)

        # 构造 Q 输入
        input_q = paddle.concat([states, h_state_action_encode], axis=-1)
...
class QTRAN_alt(nn.Layer):
    ...
    def forward(self, states: Tensor, hidden_state_inputs: Tensor, actions_onehot: Tensor):
        ...
        agent_ids = paddle.eye(self.n_agents, dtype=paddle.float32).to(self.device)
        ...
        input_q = paddle.concat([repeated_states, h_state_action_encode, repeat_agent_ids], axis=-1)
