...
def _init_layer(layer, gain=np.sqrt(2), bias=0.0):
    ...
    # 初始化权重为正交矩阵
    orthogonal_init = nn.initializer.Orthogonal(gain)
    orthogonal_init(layer.weight)

    # 初始化偏置为常数
    constant_init = nn.initializer.Constant(bias)
    constant_init(layer.bias)
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            if self.actor_representation._get_name() != "Basic_Identical":
                ...
                self.actor_representation = paddle.DataParallel(self.representation)
            ...
            if self.critic_representation._get_name() != "Basic_Identical":
                ...
                self.critic_representation = paddle.DataParallel(self.representation)
            ...
            if self.aux_critic_representation._get_name() != "Basic_Identical":
                ...
                self.aux_critic_representation = paddle.DataParallel(self.representation)
...
class SACDISPolicy(Layer):
    ...
    def Qpolicy(self, observation: Union[np.ndarray, dict]):
        ...
        z = act_prob == 0.0
        z = z.float() * 1e-8
        log_action_prob = paddle.log(act_prob + z)
    ...
    def Qtarget(self, observation: Union[np.ndarray, dict]):
        ...
        log_action_prob = paddle.log(new_act_prob + z)
