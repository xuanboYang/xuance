...
from xuance.paddlepaddle.policies import CategoricalActorNet, ActorNet
from xuance.paddlepaddle.policies.core import CriticNet, BasicQhead
from xuance.paddlepaddle.utils import ModuleType, CategoricalDistribution
...
class MAAC_Policy(Layer):
    ...
    def __init__(self,
                 ...
                 n_agents: int,
                 representation_actor: LayerDict,
                 representation_critic: LayerDict,
        ...
        self.actor, self.critic = LayerDict(), LayerDict()
        ...
        if self.distributed_training:
            ...
            if self.mixer is not None:
                self.mixer = paddle.DataParallel(self.mixer)
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Optional[Tensor] = None,
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                actor_input = paddle.concat([outputs['state'], agent_ids], axis=-1)
            else:
    ...
    def get_values(self, observation: Dict[str, Tensor], agent_ids: Tensor = None, agent_key: str = None,
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                critic_input = paddle.concat([outputs['state'], agent_ids], axis=-1)
            else:
    ...
    def value_tot(self, values_n: Tensor, global_state=None):
        if global_state is not None:
            global_state = paddle.to_tensor(global_state).to(self.device)
...
class MAAC_Policy_Share(MAAC_Policy):
    ...
    def forward(self, observation: Tensor, agent_ids: Tensor,
        ...
        if self.use_rnn:
            ...
            rnn_hidden = None
        actor_critic_input = paddle.concat([outputs['state'], agent_ids], axis=-1)
...
class COMA_Policy(Layer):
    ...
    def __init__(self,
                 action_space: Optional[Dict[str, Discrete]],
                 n_agents: int,
                 representation_actor: Layer,
                 representation_critic: Layer,
                 actor_hidden_size: Sequence[int] = None,
                 critic_hidden_size: Sequence[int] = None,
                 normalize: Optional[ModuleType] = None,
                 initialize: Optional[Callable[..., Tensor]] = None,
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                ...
                if self.critic_representation[key]._get_name() != "Basic_Identical":
                    self.critic_representation[key] = paddle.DataParallel(self.critic_representation[key])
                self.actor[key] = paddle.DataParallel(self.actor[key])
            self.critic = paddle.DataParallel(self.critic)
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Optional[Tensor] = None,
        ...
        for key in agent_list:
            ...
            act_probs[key] = nn.functional.softmax(pi_logits[key], axis=-1)
    ...
    def get_values(self, state: Tensor, observation: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        if self.use_rnn:
            # 插入新维度并扩展形状为 (batch_size, T, N, dim_S)
            expanded_state = paddle.unsqueeze(state, axis=-2)  # 形状变为 (batch_size, T, 1, dim_S)
            expand_shape = list(expanded_state.shape)  # 获取当前形状
            expand_shape[-2] = self.n_agents  # 设置 agent 维度为 self.n_agents
            expanded_state = paddle.expand(expanded_state, shape=expand_shape)  # 扩展形状
            critic_inputs.append(expanded_state)  # 添加到 critic_inputs 列表中
        else:
            # 插入新维度并扩展形状为 (batch_size, N, dim_S)
            expanded_state = paddle.unsqueeze(state, axis=-2)  # 形状变为 (batch_size, 1, dim_S)
            expand_shape = list(expanded_state.shape)  # 获取当前形状
            expand_shape[-2] = self.n_agents  # 设置 agent 维度为 self.n_agents
            expanded_state = paddle.expand(expanded_state, shape=expand_shape)  # 扩展形状
            critic_inputs.append(expanded_state)  # 添加到 critic_inputs 列表中
        ...
        agent_mask = (1 - paddle.eye(self.n_agents, dtype=paddle.float32).to(device=self.device)).unsqueeze(-1)
        if self.use_parameter_sharing:
            key = self.model_keys[0]
            # agent_mask = agent_mask.repeat(1, 1, self.n_actions[key]).reshape((self.n_agents, -1)).unsqueeze(0)
            # 使用 paddle.tile 替代 repeat
            agent_mask = paddle.tile(agent_mask, repeat_times=[1, 1, self.n_actions[key]])
            # 使用 paddle.reshape 替代 reshape
            agent_mask = paddle.reshape(agent_mask, shape=[self.n_agents, -1])
            # 使用 paddle.unsqueeze 替代 unsqueeze
            agent_mask = paddle.unsqueeze(agent_mask, axis=0)

            if self.use_rnn:
                # actions_input = actions[key].reshape((batch_size, seq_len, 1, -1)).repeat(1, 1, self.n_agents, 1)
                # 假设 actions[key] 的形状为 (batch_size, seq_len, action_dim)
                # 使用 paddle.reshape 和 paddle.tile 替代 reshape 和 repeat

                # 调整形状为 (batch_size, seq_len, 1, action_dim)
                actions_input = paddle.reshape(actions[key], shape=(batch_size, seq_len, 1, -1))

                # 在第三个维度重复 self.n_agents 次
                tile_times = [1, 1, self.n_agents, 1]  # 定义重复次数
                actions_input = paddle.tile(actions_input, repeat_times=tile_times)

                # 此时 actions_input 的形状为 (batch_size, seq_len, self.n_agents, action_dim)
                ...
                # 调整形状为 (batch_size, 1, action_dim)
                actions_input = paddle.reshape(actions[key], shape=(batch_size, 1, -1))

                # 在第二个维度重复 self.n_agents 次
                tile_times = [1, self.n_agents, 1]  # 定义重复次数
                actions_input = paddle.tile(actions_input, repeat_times=tile_times)
            ...
            critic_inputs = paddle.concat(critic_inputs, axis=-1)
        else:
            # agent_mask = paddle.concat([agent_mask[i].repeat(1, self.n_actions[k])
            #                             for i, k in enumerate(self.model_keys)], axis=-1).unsqueeze(0)
            # 使用列表推导式对每个 agent_mask[i] 进行 repeat 操作
            repeated_masks = [paddle.tile(agent_mask[i], repeat_times=[1, self.n_actions[k]])
                              for i, k in enumerate(self.model_keys)]
            # 按最后一个维度拼接所有扩展后的张量
            concatenated_mask = paddle.concat(repeated_masks, axis=-1)
            # 在第 0 维插入新维度
            agent_mask = paddle.unsqueeze(concatenated_mask, axis=0)

            if self.use_rnn:
                agent_mask = agent_mask.unsqueeze(1)
                # actions_input = paddle.concat(itemgetter(*self.model_keys)(actions),
                #                               axis=-1).unsqueeze(-2).repeat(1, 1, self.n_agents, 1)  # batch * T * N * A
                extracted_actions = itemgetter(*self.model_keys)(actions)
                # 将提取出的张量按最后一个维度拼接
                concatenated_actions = paddle.concat(extracted_actions, axis=-1)
                # 在倒数第二个维度插入新维度
                actions_input = paddle.unsqueeze(concatenated_actions, axis=-2)
                # 在第三个维度重复 self.n_agents 次
                tile_times = [1, 1, self.n_agents, 1]  # 定义重复次数
                actions_input = paddle.tile(actions_input, repeat_times=tile_times)
                ...
                concatenated_actions = paddle.concat(extracted_actions, axis=-1)
                # 在第 1 维插入新维度
                actions_input = paddle.unsqueeze(concatenated_actions, axis=1)
                # 在第二个维度重复 self.n_agents 次
                tile_times = [1, self.n_agents, 1]  # 定义重复次数
                actions_input = paddle.tile(actions_input, repeat_times=tile_times)

                agent_ids = agent_ids.reshape((batch_size, self.n_agents, -1))  # batch_size * N * N
            critic_inputs.append(paddle.stack(itemgetter(*self.model_keys)(obs_rep), axis=-2))
            critic_inputs.append(actions_input * agent_mask)
...
class MeanFieldActorCriticPolicy(Layer):
    def __init__(self,
                 action_space: Discrete,
                 n_agents: int,
                 representation: Layer,
    ...
    def forward(self, observation: Tensor, agent_ids: Tensor):
        outputs = self.representation(observation)
        input_actor = paddle.concat([outputs['state'], agent_ids], axis=-1)
    ...
    def critic(self, observation: Tensor, actions_mean: Tensor, agent_ids: Tensor):
        outputs = self.representation(observation)
        critic_in = paddle.concat([outputs['state'], actions_mean, agent_ids], axis=-1)
        return self.critic_net(critic_in)


class Basic_ISAC_Policy(Layer):
    """
    ...
    def __init__(self,
                 action_space: Optional[Dict[str, Discrete]],
                 n_agents: int,
                 actor_representation: LayerDict,
                 critic_representation: LayerDict,
        ...
        self.actor, self.critic_1, self.critic_2 = LayerDict(), LayerDict(), LayerDict()
        for key in self.model_keys:
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                ...
                if self.critic_1_representation[key]._get_name() != "Basic_Identical":
                    self.critic_1_representation[key] = paddle.DataParallel(self.critic_1_representation[key])
                if self.critic_2_representation[key]._get_name() != "Basic_Identical":
                    self.critic_2_representation[key] = paddle.DataParallel(self.critic_2_representation[key])
                self.actor[key] = paddle.DataParallel(self.actor[key])
                self.critic_1[key] = paddle.DataParallel(self.critic_1[key])
                self.critic_2[key] = paddle.DataParallel(self.critic_2[key])
    ...
    def Qpolicy(self, observation: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                actor_in = paddle.concat([actor_in, agent_ids], axis=-1)
                critic_1_in = paddle.concat([critic_1_in, agent_ids], axis=-1)
                critic_2_in = paddle.concat([critic_2_in, agent_ids], axis=-1)
            ...
            z = z.float() * 1e-8
            log_action_prob_dict[key] = paddle.log(act_prob_dict[key] + z)
    ...
    def Qtarget(self, next_observation: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            log_action_prob_dict[key] = paddle.log(new_act_prob_dict[key] + z)

            target_q_1, target_q_2 = self.target_critic_1[key](critic_1_in), self.target_critic_2[key](critic_2_in)
            target_q[key] = paddle.min(target_q_1, target_q_2)
        return (rnn_hidden_actor_new, rnn_hidden_critic_new_1, rnn_hidden_critic_new_2,
                new_act_prob_dict, log_action_prob_dict, target_q)
...
class MASAC_Policy(Basic_ISAC_Policy):
            ...
            if self.use_parameter_sharing:
                if self.use_rnn:
                    ...
                    joint_rep_out_2 = outputs_critic_2[key]['state'].unsqueeze(1).expand(
                        -1, self.n_agents, -1).reshape(bs, -1)
                critic_1_in = paddle.concat([joint_rep_out_1, agent_ids], axis=-1)
                critic_2_in = paddle.concat([joint_rep_out_2, agent_ids], axis=-1)
            else:
                if self.use_rnn:
    ...
    def Qtarget(self, joint_observation: Optional[Tensor] = None,
        ...
        for key in agent_list:
            ...
            target_q[key] = paddle.min(q_1, q_2)
