...
from xuance.paddlepaddle.policies import BasicQhead, ActorNet, CriticNet, VDN_mixer, QMIX_FF_mixer
from xuance.paddlepaddle.utils import ModuleType
from xuance.paddlepaddle import Tensor, Layer, LayerDict, DataParallel
from paddle import distributed as dist


class BasicQnetwork(Layer):
    """
    ...
    def __init__(self,
        ...
        self.eval_Qhead, self.target_Qhead = LayerDict(), LayerDict()
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                if self.representation[key]._get_name() != "Basic_Identical":
                    self.representation[key] = paddle.DataParallel(self.representation[key])
                self.eval_Qhead[key] = paddle.DataParallel(self.eval_Qhead[key])
    ...
    def forward(self, observation: Dict[str, Tensor], agent_ids: Tensor = None,
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                q_inputs = paddle.concat([outputs['state'], agent_ids], axis=-1)
            else:
            ...
            if avail_actions is not None:
                ...
                argmax_action[key] = evalQ_detach.argmax(axis=-1, keepdim=False)
            else:
                argmax_action[key] = evalQ[key].argmax(axis=-1, keepdim=False)
...
class MixingQnetwork(BasicQnetwork):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            # 初始化分布式环境
            dist.init_parallel_env()
            self.eval_Qtot = paddle.DataParallel(self.eval_Qtot)
    ...
    def Q_tot(self, individual_values: Dict[str, Tensor], states: Optional[Tensor] = None):
        ...
        if self.use_parameter_sharing:
            ...
            """
            individual_inputs = paddle.concat([individual_values[k] for k in self.model_keys],
    ...
    def Qtarget_tot(self,
        ...
        if self.use_parameter_sharing:
            ...
            From dict to tensor. For example: 
                ...
                individual_inputs: batch * 2 * 1
            """
            individual_inputs = paddle.concat([individual_values[k] for k in self.model_keys], axis=-1).reshape(
                [-1, self.n_agents, 1])
...
class Weighted_MixingQnetwork(MixingQnetwork):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            ...
            dist.init_parallel_env()
            for key in self.model_keys:
                self.eval_Qhead_centralized[key] = paddle.DataParallel(self.eval_Qhead_centralized[key])
            self.ff_mixer = paddle.DataParallel(self.ff_mixer)
...
class Qtran_MixingQnetwork(BasicQnetwork):
    ...
    def __init__(self,
        ...
        if self.distributed_training:
            dist.init_parallel_env()
            self.qtran_net = paddle.DataParallel(self.qtran_net)
            self.q_tot = paddle.DataParallel(self.q_tot)
    ...
    def Q_tran(self, states: Tensor, hidden_states: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                actions_onehot = paddle.reshape(actions_onehot, shape=[batch_size, self.n_agents, seq_len, -1])
                ...
                actions_onehot = paddle.reshape(actions_onehot, shape=[batch_size, self.n_agents, -1])
            ...
            if agent_mask is not None:
                if self.use_rnn:
                    # agent_mask = agent_mask[key].reshape(batch_size, self.n_agents, seq_len, 1).repeat(1, 1, 1, dim_hidden_state)
                    agent_mask = paddle.reshape(agent_mask[key], shape=[batch_size, self.n_agents, seq_len, 1])
                    agent_mask = paddle.tile(agent_mask, repeat_times=[1, 1, 1, dim_hidden_state])
                else:
                    # agent_mask = agent_mask[key].reshape(batch_size, self.n_agents, 1).repeat(1, 1, dim_hidden_state)
                    agent_mask = paddle.reshape(agent_mask[key], shape=[batch_size, self.n_agents, 1])
                    agent_mask = paddle.tile(agent_mask, repeat_times=[1, 1, dim_hidden_state])

                hidden_states_input = hidden_states_input * agent_mask
            if self.use_rnn:
                # states = states.reshape(batch_size * seq_len, -1)
                states = paddle.reshape(states, shape=[batch_size * seq_len, -1])

                # hidden_states_input = hidden_states_input.transpose(1, 2).reshape(-1, self.n_agents, dim_hidden_state)
                hidden_states_input = paddle.transpose(hidden_states_input, perm=[0, 2, 1])
                hidden_states_input = paddle.reshape(hidden_states_input, shape=[-1, self.n_agents, dim_hidden_state])

                # actions_onehot = actions_onehot.transpose(1, 2).reshape(-1, self.n_agents, self.n_actions_max)
                actions_onehot = paddle.transpose(actions_onehot, perm=[0, 2, 1])
                actions_onehot = paddle.reshape(actions_onehot, shape=[-1, self.n_agents, self.n_actions_max])
        else:
            hidden_states_input = paddle.concat([hidden_states[k].unsqueeze(1) for k in self.model_keys], axis=1)
            actions_onehot = paddle.concat([one_hot(actions[k].astype("int64"), self.n_actions_max).unsqueeze(1)
    ...
    def Q_tran_target(self, states: Tensor, hidden_states: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                # hidden_states_input = hidden_states[key].reshape([-1, self.n_agents, seq_len, dim_hidden_state])
                actions_onehot = paddle.reshape(actions_onehot, shape=[batch_size, self.n_agents, seq_len, -1])
                hidden_states_input = paddle.reshape(hidden_states[key],
                                                     shape=[-1, self.n_agents, seq_len, dim_hidden_state])
            else:
                # actions_onehot = actions_onehot.reshape(batch_size, self.n_agents, -1)
                # hidden_states_input = hidden_states[key].reshape([-1, self.n_agents, dim_hidden_state])
                actions_onehot = paddle.reshape(actions_onehot, shape=[batch_size, self.n_agents, -1])
                hidden_states_input = paddle.reshape(hidden_states[key],
                                                     shape=[-1, self.n_agents, dim_hidden_state])
...
class DCG_policy(Layer):
    ...
    def __init__(self,
                 ...
                 utility: Optional[Layer] = None,
                 payoffs: Optional[Layer] = None,
                 dcgraph: Optional[Layer] = None,
    ...
    def get_hidden_states(self, batch_size: int,
        ...
        if self.use_parameter_sharing:
            ...
            hidden_states_n = paddle.reshape(hidden_states[self.model_keys[0]],
                                             shape=[batch_size, self.n_agents, seq_len, -1])
            if self.use_rnn:
                # hidden_states_n = hidden_states_n.transpose(1, 2).reshape(batch_size, seq_len, self.n_agents, -1)
                hidden_states_n = paddle.transpose(hidden_states_n, perm=[0, 2, 1, 3])
                hidden_states_n = paddle.reshape(hidden_states_n, shape=[batch_size, seq_len, self.n_agents, -1])

            else:
                # hidden_states_n = hidden_states_n.transpose(1, 2).reshape(batch_size, self.n_agents, -1)
                hidden_states_n = paddle.transpose(hidden_states_n, perm=[0, 2, 1, 3])
                hidden_states_n = paddle.reshape(hidden_states_n, shape=[batch_size, self.n_agents, -1])
        else:
            if self.use_rnn:
                hidden_states_n = paddle.stack(itemgetter(*self.model_keys)(hidden_states), axis=-2)
...
class MFQnetwork(Layer):
    ...
    def forward(self, observation: Tensor, actions_mean: Tensor, agent_ids: Tensor):
        ...
        q_inputs = paddle.concat([outputs['state'], actions_mean, agent_ids], axis=-1)
...
class Independent_DDPG_Policy(Layer):
    """
    ...
    def __init__(self,
                 ...
                 n_agents: int,
                 actor_representation: Optional[LayerDict],
                 critic_representation: Optional[LayerDict],
        ...
        self.actor, self.target_actor = LayerDict(), LayerDict()
        self.critic, self.target_critic = LayerDict(), LayerDict()
        for key in self.model_keys:
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                if self.actor_representation[key]._get_name() != "Basic_Identical":
                    self.actor_representation[key] = paddle.DataParallel(self.actor_representation[key])
                if self.critic_representation[key]._get_name() != "Basic_Identical":
                    self.critic_representation[key] = paddle.DataParallel(self.critic_representation[key])
                self.actor[key] = paddle.DataParallel(self.actor[key])
                self.critic[key] = paddle.DataParallel(self.critic[key])
    ...
    def forward(self, observation: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                actor_in = paddle.concat([outputs['state'], agent_ids], axis=-1)
            else:
    ...
    def Qpolicy(self, observation: Dict[str, Tensor], actions: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                critic_in = paddle.concat([outputs['state'], agent_ids], axis=-1)
            else:
                critic_in = outputs['state']
            q_eval[key] = self.critic[key](paddle.concat([critic_in, actions[key]], axis=-1))
    ...
    def Qtarget(self, next_observation: Dict[str, Tensor], next_actions: Dict[str, Tensor],
        ...
        for key in agent_list:
            ...
            if self.use_parameter_sharing:
                ...
                critic_in = outputs['state']
            q_target[key] = self.target_critic[key](paddle.concat([critic_in, next_actions[key]], axis=-1))
        return rnn_hidden_new, q_target
    ...
    #         tp.data.add_(tau * ep.data)
    def soft_update(self, tau=0.005):
        def update_target(target_param, source_param, tau):
            """Update target parameter with a weighted average of source parameter."""
            updated_value = (1 - tau) * target_param.value() + tau * source_param.value()
            target_param.set_value(updated_value)

        # Update actor_representation and target_actor_representation
        for ep, tp in zip(self.actor_representation.parameters(), self.target_actor_representation.parameters()):
            update_target(tp, ep, tau)

        # Update critic_representation and target_critic_representation
        for ep, tp in zip(self.critic_representation.parameters(), self.target_critic_representation.parameters()):
            update_target(tp, ep, tau)

        # Update actor and target_actor
        for ep, tp in zip(self.actor.parameters(), self.target_actor.parameters()):
            update_target(tp, ep, tau)

        # Update critic and target_critic
        for ep, tp in zip(self.critic.parameters(), self.target_critic.parameters()):
            update_target(tp, ep, tau)
...
class MADDPG_Policy(Independent_DDPG_Policy):
    ...
    def Qpolicy(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        critic_rep_in = paddle.concat([joint_observation, joint_actions], axis=-1)
        if self.use_rnn:
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                ...
                critic_in = paddle.concat([joint_rep_out, agent_ids], axis=-1)
            else:
                if self.use_rnn:
...
class MATD3_Policy(MADDPG_Policy, Layer):
    """
    The policy of deep deterministic policy gradient.
    ...
    def __init__(self,
        ...
        self.actor, self.target_actor = LayerDict(), LayerDict()
        self.critic_A, self.critic_B = LayerDict(), LayerDict()
        self.target_critic_A, self.target_critic_B = LayerDict(), LayerDict()
        for key in self.model_keys:
            dim_action = self.action_space[key].shape[-1]
            dim_actor_in, dim_actor_out, dim_critic_in = self._get_actor_critic_input(
                self.actor_representation[key].output_shapes['state'][0], dim_action,
        ...
        if self.distributed_training:
            ...
            for key in self.model_keys:
                ...
                if self.critic_A_representation[key]._get_name() != "Basic_Identical":
                    self.critic_A_representation[key] = paddle.DataParallel(self.critic_A_representation[key])
                if self.critic_B_representation[key]._get_name() != "Basic_Identical":
                    self.critic_B_representation[key] = paddle.DataParallel(self.critic_B_representation[key])
                self.actor[key] = paddle.DataParallel(self.actor[key])
                self.critic_A[key] = paddle.DataParallel(self.critic_A[key])
                self.critic_B[key] = paddle.DataParallel(self.critic_B[key])
    ...
    def Qpolicy(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        for key in agent_list:
            if self.use_parameter_sharing:
                ...
                critic_in_A = paddle.concat([joint_rep_out_A, agent_ids], axis=-1)
                critic_in_B = paddle.concat([joint_rep_out_B, agent_ids], axis=-1)
            else:
                if self.use_rnn:
    ...
    def Qtarget(self, joint_observation: Tensor, joint_actions: Tensor,
        ...
        for key in agent_list:
            ...
            q_target[key] = paddle.minimum(q_target_A, q_target_B)
