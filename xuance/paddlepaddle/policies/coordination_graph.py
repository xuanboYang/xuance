...
class DCG_payoff(DCG_utility):
    ...
    def forward(self, hidden_states_n: Tensor,
        ...
        """
        input_payoff = paddle.stack(
            [paddle.concat([hidden_states_n[:, edges_from], hidden_states_n[:, edges_to]], axis=-1),
             paddle.concat([hidden_states_n[:, edges_to], hidden_states_n[:, edges_from]], axis=-1)],
            axis=0)
        ...
        if self.low_rank_payoff:
            ...
            payoffs = paddle.matmul(payoffs[:, 0, :].unsqueeze(dim=-1), payoffs[:, 1, :].unsqueeze(
                dim=-2))  # (dim_act * 1) * (1 * dim_act) -> (dim_act * dim_act)
            payoffs = payoffs.view(list(dim) + [self.payoff_rank, self.dim_act, self.dim_act]).sum(axis=-3)
        else:
        ...
        payoffs[1] = paddle.transpose(payoffs[1], perm=[-2, -1]).clone()  # f_ij(a_i, a_j) <-> f_ji(a_j, a_i)
...
class Coordination_Graph(object):
    ...
    def set_coordination_graph(self):
        ...
        self.edges_from = paddle.zeros([self.n_edges], dtype='int64').to(self.device)
        self.edges_to = paddle.zeros([self.n_edges], dtype='int64').to(self.device)
