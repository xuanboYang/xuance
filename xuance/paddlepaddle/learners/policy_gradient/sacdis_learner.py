...
class SACDIS_Learner(Learner):
    def __init__(self,
        ...
        if self.use_automatic_entropy_tuning:
            ...
            self.log_alpha = paddle.create_parameter(
                shape=[1],  # 参数形状为 [1]
                default_initializer=nn.initializer.Constant(value=0.0),  # 初始化为 0
                dtype='float32'  # 数据类型为 float32
            )
            self.log_alpha.stop_gradient = False  # 设置为需要梯度计算
            ...
            self.alpha_optimizer = paddle.optimizer.Adam(
                learning_rate=self.config.learning_rate_actor,
                parameters=[self.log_alpha]  # 将 log_alpha 添加到优化器中
            )
    ...
    def update(self, **samples):
        ...
        act_batch = paddle.to_tensor(samples['actions']).to(self.device).unsqueeze(-1)
        next_batch = paddle.to_tensor(samples['obs_next']).to(self.device)
        rew_batch = paddle.to_tensor(samples['rewards']).to(self.device).unsqueeze(-1)
        ter_batch = paddle.to_tensor(samples['terminals'], dtype=paddle.float32).to(self.device).reshape([-1, 1])

        # actor update
        action_prob, log_pi, policy_q_1, policy_q_2 = self.policy.Qpolicy(obs_batch)
        policy_q = paddle.min(policy_q_1, policy_q_2)
        ...
        target_q = target_q.sum(axis=1).unsqueeze(-1)
