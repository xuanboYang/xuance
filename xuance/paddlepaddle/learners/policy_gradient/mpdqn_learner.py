...
class MPDQN_Learner(Learner):
    def __init__(self,
        ...
        #                                                           total_iters=self.config.running_steps)
        # 定义优化器
        conactor_optimizer = Adam(
            learning_rate=self.config.learning_rate,
            parameters=self.policy.conactor.parameters()
        )
        qnetwork_optimizer = Adam(
            learning_rate=self.config.learning_rate,
            parameters=self.policy.qnetwork.parameters()
        )
        self.optimizers = [conactor_optimizer, qnetwork_optimizer]

        # 定义学习率调度器
        conactor_lr_scheduler = LinearWarmup(
            learning_rate=self.config.learning_rate,
            warmup_steps=self.config.running_steps,
            start_lr=self.config.learning_rate * 1.0,
            end_lr=self.config.learning_rate * self.end_factor_lr_decay,
            verbose=False
        )

        qnetwork_lr_scheduler = LinearWarmup(
            learning_rate=self.config.learning_rate,
            warmup_steps=self.config.running_steps,
            start_lr=self.config.learning_rate * 1.0,
            end_lr=self.config.learning_rate * self.end_factor_lr_decay,
    ...
    def update(self, **samples):
        ...
        hyact_batch = paddle.to_tensor(samples['actions']).to(self.device)
        ...
        with torch.no_grad():
            ...
            target_q = paddle.max(target_q, 1, keepdim=True)[0].squeeze()
