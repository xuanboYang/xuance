...
class MADDPG_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.optimizer = {
            key: {
                'actor': optim.Adam(parameters=self.policy.parameters_actor[key],
                                    learning_rate=self.config.learning_rate_actor,
                                    epsilon=1e-5),
                'critic': optim.Adam(parameters=self.policy.parameters_critic[key],
        ...
        self.scheduler = {
            key: {
                'actor': lr_scheduler.LinearLR(
                    learning_rate=self.optimizer[key]['actor'].get_lr(),
                    start_factor=self.optimizer[key]['actor'].get_lr(),
                    end_factor=self.optimizer[key]['actor'].get_lr() * self.end_factor_lr_decay,
                    total_steps=self.config.running_steps,
                    verbose=False
                ),
                'critic': lr_scheduler.LinearLR(
                    learning_rate=self.optimizer[key]['critic'].get_lr(),
                    start_factor=self.optimizer[key]['critic'].get_lr(),
                    end_factor=self.optimizer[key]['critic'].get_lr() * self.end_factor_lr_decay,
                    total_steps=self.config.running_steps,
                    verbose=False
                )
            }
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            bs = batch_size
            obs_joint = paddle.concat(itemgetter(*self.agent_keys)(obs), axis=-1).reshape((batch_size, -1))
            next_obs_joint = paddle.concat(itemgetter(*self.agent_keys)(obs_next), axis=-1).reshape((batch_size, -1))
            actions_joint = paddle.concat(itemgetter(*self.agent_keys)(actions), axis=-1).reshape((batch_size, -1))
        ...
        if self.use_parameter_sharing:
            ...
            actions_next_joint = paddle.concat(itemgetter(*self.model_keys)(actions_next), -1).reshape((batch_size, -1))
        ...
        for key in self.model_keys:
            mask_values = agent_mask[key].astype('float32')
            # update critic
            ...
            if self.use_parameter_sharing:
                ...
                act_eval = paddle.concat(itemgetter(*self.agent_keys)(a_joint), axis=-1).reshape((batch_size, -1))
            ...
            print(f"maddpg, self.optimizer[key]['actor'].state_dict(): {self.optimizer[key]['actor'].state_dict()}")
            print(f"maddpg, self.optimizer[key]['critic'].state_dict(): {self.optimizer[key]['critic'].state_dict()}")
            learning_rate_actor = self.optimizer[key]['actor'].get_lr()  # self.optimizer[key]['actor'].state_dict()['param_groups'][0]['lr']
            learning_rate_critic = self.optimizer[key]['critic'].get_lr()  # self.optimizer[key]['critic'].state_dict()['param_groups'][0]['lr']
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            obs_joint = paddle.concat(itemgetter(*self.agent_keys)(obs), axis=-1).reshape((batch_size, seq_len + 1, -1))
            actions_joint = paddle.concat(itemgetter(*self.agent_keys)(actions), axis=-1).reshape(
        ...
        if self.use_parameter_sharing:
            ...
            actions_next_joint = paddle.concat(itemgetter(*self.agent_keys)(actions_next),
                                               axis=-1).reshape((batch_size, seq_len + 1, -1))
        ...
        for key in self.model_keys:
            ...
            if self.use_parameter_sharing:
                ...
                act_eval = paddle.concat(itemgetter(*self.agent_keys)(a_dict), axis=-1).reshape(
                    (batch_size, seq_len, -1))
