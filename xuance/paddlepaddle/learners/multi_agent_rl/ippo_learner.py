...
class HuberLoss(nn.Layer):
    def __init__(self, delta=1.0, reduction='mean'):
        """
        初始化 Huber Loss
        :param delta: 阈值参数，控制误差从 MSE 切换到 MAE 的点，默认为 1.0
        :param reduction: 损失的规约方式，可选 'none', 'mean', 'sum'
        """
    ...
    def forward(self, input, target):
        """
        前向传播计算 Huber Loss
        :param input: 预测值，形状为 [N, *]
        :param target: 真实值，形状为 [N, *]
        :return: 计算得到的 Huber Loss
        """
        # 计算误差
        error = paddle.abs(input - target)

        # 根据 delta 判断误差范围，分别计算 MSE 和 MAE
        quadratic = paddle.minimum(error, paddle.to_tensor(self.delta))
        linear = error - quadratic
        loss = 0.5 * quadratic ** 2 + self.delta * linear

        # 根据 reduction 参数进行损失规约
...
class IPPO_Learner(IAC_Learner):
        ...
        if self.use_value_norm:
            ...
            for key in self.agent_keys:
                if self.use_parameter_sharing:
                    ...
                    if key not in self.value_normalizer:
                        self.value_normalizer[key] = ValueNorm(1).to(self.device)
                else:
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            ratio = paddle.exp(log_pi - log_pi_old[key]).reshape(bs)
            advantages_mask = advantages[key].detach() * mask_values
            surrogate1 = ratio * advantages_mask
            surrogate2 = paddle.clip(ratio, 1 - self.clip_range, 1 + self.clip_range) * advantages_mask
            loss_a.append(-paddle.min(surrogate1, surrogate2).mean())
        ...
        if self.use_grad_clip:
            grad_norm = paddle.nn.utils.clip_grad_norm_(self.policy.parameters_model, self.grad_clip_norm)
    ...
    def update_rnn(self, sample):
        ...
        for key in self.model_keys:
            ...
            ratio = paddle.exp(log_pi - log_pi_old[key])
            surrogate1 = ratio * advantages[key]
            surrogate2 = paddle.clip(ratio, 1 - self.clip_range, 1 + self.clip_range) * advantages[key]
            loss_a.append(-(paddle.min(surrogate1, surrogate2) * mask_values).sum() / mask_values.sum())
