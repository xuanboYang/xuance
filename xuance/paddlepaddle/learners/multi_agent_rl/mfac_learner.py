...
class MFAC_Learner(LearnerMAS):
    ...
    def update(self, sample):
        info = {}
        self.iterations += 1
        state = paddle.Tensor(sample['state']).to(self.device)
        ...
        returns = paddle.Tensor(sample['returns']).to(self.device)
        agent_mask = paddle.Tensor(sample['agent_mask']).float().reshape(-1, self.n_agents, 1).to(self.device)
        batch_size = obs.shape[0]
        IDs = paddle.eye(self.n_agents).unsqueeze(0).expand((batch_size, -1, -1)).to(self.device)
