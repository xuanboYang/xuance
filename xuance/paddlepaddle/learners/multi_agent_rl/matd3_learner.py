...
class MATD3_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.scheduler = {
            key: {
                'actor': lr_scheduler.LinearWarmup(
                    learning_rate=self.optimizer[key]['actor'].get_lr(),
                    warmup_steps=0,  # No warmup steps
                    start_lr=self.optimizer[key]['actor'].get_lr(),
                    end_lr=self.optimizer[key]['actor'].get_lr() * self.end_factor_lr_decay,
                    total_steps=self.config.running_steps
                ),
                'critic': lr_scheduler.LinearWarmup(
                    learning_rate=self.optimizer[key]['critic'].get_lr(),
                    warmup_steps=0,  # No warmup steps
                    start_lr=self.optimizer[key]['critic'].get_lr(),
                    end_lr=self.optimizer[key]['critic'].get_lr() * self.end_factor_lr_decay,
                    total_steps=self.config.running_steps
                )
            }
            for key in self.model_keys
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            next_actions_joint = paddle.concat(itemgetter(*self.agent_keys)(actions_next),
        ...
        if self.iterations % self.actor_update_delay == 0:
            ...
            for key in self.model_keys:
                ...
                if self.use_parameter_sharing:
                    ...
                    act_eval = paddle.concat(itemgetter(*self.agent_keys)(a_joint), axis=-1).reshape(
                        (batch_size, seq_len,
                         -1))
