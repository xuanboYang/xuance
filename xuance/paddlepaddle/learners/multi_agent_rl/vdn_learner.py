...
class VDN_Learner(LearnerMAS):
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            terminals_tot = paddle.all(terminals[key], axis=1, keepdim=True).astype('float32')
        else:
            bs = batch_size
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            terminals_tot = paddle.all(terminals[key], axis=1, keepdim=True).astype('float32').reshape([-1, 1])
        else:
            bs_rnn = batch_size
            rewards_list = itemgetter(*self.agent_keys)(rewards)
        ...
        for key in self.model_keys:
            q_eval_a[key] = paddle.take_along_axis(
                q_eval[key][:, :-1], actions[key].astype('int64').unsqueeze(-1), axis=-1).reshape([bs_rnn, seq_len])
            ...
            if self.config.double_q:
                ...
                q_next_a[key] = paddle.take_along_axis(
                    q_next[key], act_next[key].astype('int64').detach(), axis=-1).reshape([bs_rnn, seq_len])
            else:
