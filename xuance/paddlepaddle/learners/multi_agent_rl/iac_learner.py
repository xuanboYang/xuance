...
from xuance.paddlepaddle import Tensor
from xuance.paddlepaddle.learners.multi_agent_rl.HuberLoss import HuberLoss
from xuance.paddlepaddle.utils import ValueNorm
...
class IAC_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.huber_loss = HuberLoss(delta=self.huber_delta, reduction='none')
    ...
    def build_optimizer(self):
        ...
        self.optimizer = paddle.optimizer.Adam(
            ...
            epsilon=1e-5,
            weight_decay=0.01  # 注意这里是浮点数，而不是整数
        )
        ...
        class LinearLRScheduler:
            ...
            def step(self, current_iter):
                if current_iter < self.total_iters:
                    ...
                    new_lr = self.optimizer.get_lr() * factor
                    self.optimizer.set_lr(new_lr)

        # 初始化线性学习率调度器
        self.scheduler = LinearLRScheduler(
    ...
    def build_training_data(self, sample: Optional[dict],
        ...
        if use_parameter_sharing:
            ...
            # 检查 'advantages' 键是否存在于 sample 中
            if 'advantages' in sample:
                # 检查是否所有agent_keys都在sample['advantages']中
                if all(key in sample['advantages'] for key in self.agent_keys):
                    advantages_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample['advantages']), 1)).to(self.device)
                else:
                    # 如果缺少某些键，创建一个全零的advantages值
                    advantages_shape = returns_tensor.shape
                    advantages_tensor = paddle.zeros(advantages_shape, dtype=returns_tensor.dtype).to(self.device)
            else:
                # 如果sample中没有advantages键，创建一个全零的advantages值
                advantages_shape = returns_tensor.shape
                advantages_tensor = paddle.zeros(advantages_shape, dtype=returns_tensor.dtype).to(self.device)
                
            # 检查 'log_pi_old' 键是否存在于 sample 中
            if 'log_pi_old' in sample:
                # 检查是否所有agent_keys都在sample['log_pi_old']中
                if all(key in sample['log_pi_old'] for key in self.agent_keys):
                    log_pi_old_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample['log_pi_old']), 1)).to(self.device)
                else:
                    # 如果缺少某些键，创建一个全零的log_pi_old值
                    log_pi_old_shape = returns_tensor.shape
                    log_pi_old_tensor = paddle.zeros(log_pi_old_shape, dtype=returns_tensor.dtype).to(self.device)
            else:
                # 如果sample中没有log_pi_old键，创建一个全零的log_pi_old值
                log_pi_old_shape = returns_tensor.shape
                log_pi_old_tensor = paddle.zeros(log_pi_old_shape, dtype=returns_tensor.dtype).to(self.device)
            
            ter_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample['terminals']), 1)).astype("float32").to(
                self.device)
            msk_tensor = Tensor(np.stack(itemgetter(*self.agent_keys)(sample['agent_mask']), 1)).astype("float32").to(
                self.device)
            if self.use_rnn:
                ...
                IDs = paddle.eye(self.n_agents).unsqueeze(1).unsqueeze(0).expand(
                ...
                IDs = paddle.eye(self.n_agents).unsqueeze(0).expand(
            ...
            if use_actions_mask:
                ...
                if self.use_rnn:
                    avail_actions = {k: Tensor(avail_a.reshape([bs, seq_length, -1])).astype("float32").to(self.device)}
                else:
                    avail_actions = {k: Tensor(avail_a.reshape([bs, -1])).astype("float32").to(self.device)}
            ...
            if 'advantages' in sample:
                ...
                if all(key in sample['advantages'] for key in self.agent_keys):
                    ...
                    # 如果缺少某些键，创建一个全零的advantages值
                    advantages = {k: paddle.zeros_like(returns[k]).to(self.device) for k in self.agent_keys}
            else:
                # 如果sample中没有advantages键，创建一个全零的advantages值
                advantages = {k: paddle.zeros_like(returns[k]).to(self.device) for k in self.agent_keys}
            ...
            if 'log_pi_old' in sample:
                ...
                if all(key in sample['log_pi_old'] for key in self.agent_keys):
                    ...
                    # 如果缺少某些键，创建一个全零的log_pi_old值
                    log_pi_old_shape = returns[k].shape
                    log_pi_old = {k: paddle.zeros(log_pi_old_shape, dtype=returns[k].dtype).to(self.device) for k in self.agent_keys}
            else:
                # 如果sample中没有log_pi_old键，创建一个全零的log_pi_old值
                log_pi_old = {k: paddle.zeros_like(returns[k]).to(self.device) for k in self.agent_keys}
                
            terminals = {k: Tensor(sample['terminals'][k]).astype("float32").to(self.device) for k in self.agent_keys}
            agent_mask = {k: Tensor(sample['agent_mask'][k]).astype("float32").to(self.device) for k in self.agent_keys}
            if use_actions_mask:
                avail_actions = {k: Tensor(sample['avail_actions'][k]).astype("float32").to(self.device) for k in
                                 self.agent_keys}
        ...
        if self.use_rnn:
            filled = Tensor(sample['filled']).astype("float32").to(self.device)
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            if self.use_value_clip:
                ...
                loss_c_ = paddle.max(loss_v, loss_v_clipped) * mask_values
