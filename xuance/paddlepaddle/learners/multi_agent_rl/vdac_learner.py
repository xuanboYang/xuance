...
class VDAC_Learner(IAC_Learner):
    ...
    def update(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            values_n = paddle.concat(itemgetter(*self.agent_keys)(values_pred_individual), axis=-1)
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            values_n = paddle.stack(itemgetter(*self.agent_keys)(values_pred_individual),
                                    axis=2).reshape((-1, self.n_agents))
        if self.config.mixer == "VDN":
            values_tot = self.policy.value_tot(values_n)
        elif self.config.mixer == "QMIX":
            values_tot = self.policy.value_tot(values_n, state)
