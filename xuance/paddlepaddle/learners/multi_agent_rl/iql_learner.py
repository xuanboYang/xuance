...
class IQL_Learner(LearnerMAS):
    def __init__(self,
        ...
        self.optimizer = {key: paddle.optimizer.Adam(parameters=self.policy.parameters_model[key],
                                                     learning_rate=config.learning_rate,
                                                     epsilon=1e-5) for key in self.model_keys}
        self.scheduler = {key: paddle.optimizer.lr.LinearWarmup(learning_rate=config.learning_rate,
                                                                warmup_steps=self.config.running_steps // 10,
                                                                start_lr=0.0,
                                                                end_lr=config.learning_rate,
                                                                verbose=False) for key in self.model_keys}
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            # q_eval_a = q_eval[key].gather(-1, actions[key].astype('int64').unsqueeze(-1)).reshape([bs])
            actions_index = actions[key].astype('int64').unsqueeze(-1)  # 转换为形状 [batch_size * n_agents, 1]

            # 使用 take_along_axis 沿最后一个维度提取值
            q_eval_a = paddle.take_along_axis(q_eval[key], actions_index, axis=-1).reshape([bs])
            ...
            if self.config.double_q:
                ...
                # q_next_a = q_next[key].gather(-1, actions_next_greedy[key].unsqueeze(-1).astype('int64')).reshape([bs])
                actions_index = actions_next_greedy[key].astype('int64').unsqueeze(
                    -1)  # 转换为形状 [batch_size * n_agents, 1]
                # 使用 take_along_axis 沿最后一个维度提取值
                q_next_a = paddle.take_along_axis(q_next[key], actions_index, axis=-1).reshape([bs])

            else:
                q_next_a = q_next[key].max(axis=-1, keepdim=True).values.reshape([bs])

            q_target = rewards[key] + (1 - terminals[key]) * self.gamma * q_next_a
            # calculate the loss function
            td_error = (q_eval_a - q_target.detach()) * (agent_mask[key].astype('float32'))
            loss = (td_error ** 2).sum() / agent_mask[key].sum()

            self.optimizer[key].clear_grad()
            loss.backward()
            if self.use_grad_clip:
                paddle.nn.utils.clip_grad_norm_(self.policy.parameters_model[key], self.grad_clip_norm)
            self.optimizer[key].step()
            if self.scheduler[key] is not None:
                self.scheduler[key].step()

            lr = self.optimizer[key].get_lr()
    ...
    def update_rnn(self, sample):
        ...
        for key in self.model_keys:
            q_eval_a = q_eval[key][:, :-1].gather(-1, actions[key].astype('int64').unsqueeze(-1)).reshape([bs_rnn, seq_len])
            ...
            if self.config.double_q:
                ...
                q_next_a = q_next.gather(-1, actions_next_greedy.astype('int64').detach()).reshape([bs_rnn, seq_len])
            else:
                q_next_a = q_next.max(axis=-1, keepdim=True).values.reshape([bs_rnn, seq_len])
