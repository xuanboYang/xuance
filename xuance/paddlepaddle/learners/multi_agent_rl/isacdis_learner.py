...
class ISACDIS_Learner(LearnerMAS):
    def __init__(self,
                 ...
                 agent_keys: List[str],
                 policy: nn.Layer):
        super(ISACDIS_Learner, self).__init__(config, model_keys, agent_keys, policy)
        # self.optimizer = {
        ...
        if self.use_automatic_entropy_tuning:
            ...
            self.alpha_optimizer = {
                key: paddle.optimizer.Adam(
                    parameters=[self.log_alpha[key]],
                    learning_rate=self.config.learning_rate_actor)
                for key in self.model_keys
            }
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            target_value = target_value.sum(axis=1).reshape(bs)
            ...
            policy_q = paddle.min(policy_q_1[key], policy_q_2[key])
            loss_a = (action_prob[key] * (self.alpha[key] * log_pi[key] - policy_q)).sum(axis=1)
