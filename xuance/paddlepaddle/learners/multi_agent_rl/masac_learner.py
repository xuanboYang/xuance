...
from xuance.paddlepaddle.learners.multi_agent_rl.isac_learner import ISAC_Learner
...
class MASAC_Learner(ISAC_Learner):
    ...
    def update(self, sample):
        ...
        for key in self.model_keys:
            ...
            if self.use_grad_clip:
                paddle.nn.utils.clip_grad_norm_(self.policy.parameters_critic[key], self.grad_clip_norm)
            ...
            if self.use_parameter_sharing:
                ...
                actions_eval_joint = paddle.concat(itemgetter(*self.model_keys)(actions_eval_detach_others),
            ...
            policy_q = paddle.min(policy_q_1[key], policy_q_2[key]).reshape(bs)
            loss_a = ((self.alpha[key] * log_pi_eval_i - policy_q) * mask_values).sum() / mask_values.sum()
            self.optimizer[key]['actor'].clear_grad()
            loss_a.backward()
            if self.use_grad_clip:
                paddle.nn.utils.clip_grad_norm_(self.policy.parameters_actor[key], self.grad_clip_norm)
            ...
            if self.use_automatic_entropy_tuning:
                ...
                self.alpha_optimizer[key].clear_grad()
                alpha_loss.backward()
    ...
    def update_rnn(self, sample):
        ...
        if self.use_parameter_sharing:
            ...
            actions_eval_joint = paddle.concat(itemgetter(*self.agent_keys)(actions_eval),
        ...
        for key in self.model_keys:
            ...
            if self.use_parameter_sharing:
                ...
                actions_eval_detach_others = {k: actions_eval[k] if k == key else actions_eval[k].detach()
                                              for k in self.model_keys}
                actions_eval_joint = paddle.concat(itemgetter(*self.model_keys)(actions_eval_detach_others),
                                                   axis=-1).reshape((batch_size, seq_len + 1, -1))[:, :-1]
            ...
            policy_q = paddle.min(policy_q_1[key], policy_q_2[key]).reshape(bs_rnn, seq_len)
            loss_a = ((self.alpha[key] * log_pi_eval_i - policy_q) * mask_values).sum() / mask_values.sum()
            self.optimizer[key]['actor'].clear_grad()
            loss_a.backward()
            if self.use_grad_clip:
