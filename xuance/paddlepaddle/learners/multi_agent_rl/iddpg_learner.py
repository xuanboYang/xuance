...
# 自定义线性学习率调度器
class LinearLR(paddle.optimizer.lr.LRScheduler):
...
class IDDPG_Learner(LearnerMAS):
    def __init__(self,
        ...
        # self.optimizer = {
        #     key: {
        #         'actor': torch.optim.Adam(self.policy.parameters_actor[key], self.config.learning_rate_actor, eps=1e-5),
        #         'critic': torch.optim.Adam(self.policy.parameters_critic[key], self.config.learning_rate_critic,
        #                                    eps=1e-5)}
        ...
        self.scheduler = {
            key: {
                'actor': LinearLR(
                    optimizer=self.optimizer[key]['actor'],
                    start_factor=1.0,
                    end_factor=self.end_factor_lr_decay,
                    total_iters=self.config.running_steps
                ),
                'critic': LinearLR(
                    optimizer=self.optimizer[key]['critic'],
                    start_factor=1.0,
                    end_factor=self.end_factor_lr_decay,
                    total_iters=self.config.running_steps
                )
