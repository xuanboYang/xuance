...
class Learner(ABC):
    def __init__(self,
        ...
        self.policy = policy
        self.optimizer: Union[dict, list, Optional[paddle.optimizer.Optimizer]] = None
        self.scheduler: Union[dict, list, Optional[paddle.optimizer.lr.LRScheduler]] = None
    ...
    def save_model(self, model_path):
        paddle.save(self.policy.state_dict(), model_path)
        if self.distributed_training:
            self.save_snapshot()
    ...
    def load_model(self, path, model=None):
        ...
        self.policy.load_state_dict(paddle.load(str(model_path), map_location={
    ...
    def load_snapshot(self, snapshot_path):
        ...
        snapshot = paddle.load(snapshot_path, map_location=loc)
    ...
    def save_snapshot(self):
        ...
        paddle.save(snapshot, snapshot_pt)
...
class LearnerMAS(ABC):
    ...
    def build_training_data(self, sample: Optional[dict],
        ...
        if use_parameter_sharing:
            ...
            if self.n_agents == 1:
                ...
                ter_tensor = Tensor(sample['terminals'][k]).astype('float32').to(self.device).unsqueeze(1)
                msk_tensor = Tensor(sample['agent_mask'][k]).astype('float32').to(self.device).unsqueeze(1)
            else:
            ...
            if use_actions_mask:
                ...
                if self.use_rnn:
                    avail_actions = {k: Tensor(avail_a.reshape([bs, seq_length + 1, -1])).astype('float32').to(self.device)}
                else:
                    ...
                    avail_actions_next = {k: Tensor(avail_a_next.reshape([bs, -1])).astype('float32').to(self.device)}
        else:
            ...
            if use_actions_mask:
                avail_actions = {k: Tensor(sample['avail_actions'][k]).astype('float32').to(self.device) for k in self.agent_keys}
                if not self.use_rnn:
                    avail_actions_next = {k: Tensor(sample['avail_actions_next'][k]).astype('float32').to(self.device) for k in
                                          self.model_keys}
    ...
    def load_model(self, path, model=None):
        ...
        print(f"保存的模型参数键值  state_dict.keys(): {state_dict.keys()}")
        for key, value in state_dict.items():
            print(f"{key}: {value.dtype}")

        # 打印当前模型的参数键值
        print(f"当前模型的参数键值  self.policy.state_dict().keys(): {self.policy.state_dict().keys()}")
        for key, param in self.policy.state_dict().items():
            print(f"{key}: {param.dtype}")

        # self.policy.set_state_dict(paddle.load(str(model_path), map_location={
        #     f"cuda:{i}": "cpu" for i in range(MAX_GPUs)}), strict=False)
        state_dict = paddle.load(str(model_path))
        model_state_dict = self.policy.state_dict()
        filtered_state_dict = {k: v for k, v in state_dict.items() if k in model_state_dict}
        self.policy.set_state_dict(filtered_state_dict)
