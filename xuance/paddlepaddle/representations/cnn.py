...
from xuance.paddlepaddle.utils import paddle, nn, cnn_block, mlp_block, ModuleType


# process the input observations with stacks of CNN layers
class Basic_CNN(Layer):
    def __init__(self,
    ...
    def forward(self, observations: np.ndarray):
        ...
        tensor_observation = paddle.to_tensor(observations, dtype=paddle.float32).permute((0, 3, 1, 2))
        return {'state': self.model(tensor_observation)}


class AC_CNN_Atari(Layer):
    def __init__(self,
                 input_shape: Sequence[int],
                 kernels: Sequence[int],
                 strides: Sequence[int],
