...
from xuance.paddlepaddle.agents import OnPolicyMARLAgents
from xuance.common.memory_tools_marl import COMA_Buffer, COMA_Buffer_RNN
...
class COMA_Agents(OnPolicyMARLAgents):
    ...
    def _build_memory(self):
        ...
        """
        # 使用特定的COMA缓冲区
        input_buffer = dict(n_agents=self.n_agents,
        ...
        print(f"使用 {Buffer.__name__} 缓冲区")
    ...
    def store_experience(self, obs_dict, avail_actions, actions_dict, log_pi_a, rewards_dict, values_dict,
        ...
        try:
            ...
            #             else:
            #                 print(f"    {agent_key}: type={type(data)}")
            #     else:
            ...
            if self.use_global_state:
                if isinstance(kwargs['state'], list):
                    # 确保state为numpy数组
                    state_array = np.array(kwargs['state'])
                    experience_data['state'] = state_array
                else:
            ...
            print(f"存储经验时出现错误: {str(e)}")
            import traceback
            traceback.print_exc()
            # 尝试使用更简单的结构
            try:
                simplified_data = {
                    'obs': {k: np.zeros((self.n_envs, 1)) for k in self.agent_keys},
                    'actions': {k: np.zeros((self.n_envs, 1), dtype=np.int32) for k in self.agent_keys},
                    'rewards': {k: np.zeros(self.n_envs) for k in self.agent_keys},
                    'values': {k: np.zeros(self.n_envs) for k in self.agent_keys},
                    'terminals': {k: np.zeros(self.n_envs, dtype=np.bool_) for k in self.agent_keys},
                    'agent_mask': {k: np.ones(self.n_envs, dtype=np.bool_) for k in self.agent_keys},
                }
                if self.use_global_state:
                    simplified_data['state'] = np.zeros((self.n_envs, 10))  # 假设状态维度为10
                self.memory.store(**simplified_data)
                print("使用简化数据结构存储成功")
            except Exception as e2:
                print(f"使用简化数据结构存储也失败: {str(e2)}")
    ...
    def action(self,
        ...
        if self.use_parameter_sharing:
            ...
            actions_sample = pi_dists[key].sample(shape=[1])
            ...
            agents_id = paddle.eye(self.n_agents).unsqueeze(0).expand(n_env, -1, -1).to(self.device)
            ...
            actions_out = paddle.stack(itemgetter(*self.agent_keys)(actions_sample), axis=-1)
        ...
        if not test_mode:  # calculate target values
            if isinstance(state, list):  # 如果 state 是列表，则先转换为 NumPy 数组
                state = np.stack(state)  # 将列表中的所有 NumPy 数组堆叠成一个统一的 NumPy 数组

            if self.use_rnn:
                state_tensor = Tensor(state).reshape((n_env, 1, -1))
                if self.use_parameter_sharing:
                ...
                state_tensor = Tensor(state).reshape((n_env, -1))  # 将 NumPy 数组转换为 Tensor

            # 确保设备名称正确
            if isinstance(self.device, str) and self.device.startswith('gpu'):
                device_str = 'gpu:0' if self.device == 'gpu' else self.device
            else:
                device_str = 'cpu'
                
            # 将tensor移动到设备上
            state_tensor = state_tensor.to(device_str)
            ...
            # 在最后一个维度插入新维度
            actions_indices = paddle.unsqueeze(actions_out, axis=-1)  # 形状变为 (n_env, self.n_agents, 1)
            # 使用 paddle.gather_nd 提取对应值
            # 构造索引张量
            batch_size = values_out.shape[0]
            agent_dim = values_out.shape[1]
            # 创建 batch 和 agent 的索引
            batch_indices = paddle.arange(batch_size).unsqueeze(-1).unsqueeze(-1).expand([batch_size, agent_dim, 1])
            agent_indices = paddle.arange(agent_dim).unsqueeze(0).unsqueeze(-1).expand([batch_size, agent_dim, 1])
            # 合并索引
            indices = paddle.concat([batch_indices, agent_indices, actions_indices],
                                    axis=-1)  # 形状为 (n_env, self.n_agents, 3)
            # 使用 gather_nd 提取值
            values_selected = paddle.gather_nd(values_out, indices)  # 形状为 (n_env, self.n_agents)
            # 调整形状（如果需要）
            values_out = values_selected.reshape((n_env, self.n_agents))
    ...
    def values_next(self,
        ...
        # 生成单位矩阵并调整形状
        agents_id = paddle.eye(self.n_agents).unsqueeze(0)  # 形状变为 (1, self.n_agents, self.n_agents)
        # 在第 0 维重复 n_env 次
        agents_id = paddle.tile(agents_id, repeat_times=[n_env, 1, 1])  # 形状变为 (n_env, self.n_agents, self.n_agents)
        # 将张量移动到指定设备
        agents_id = agents_id.to(device_str)
        ...
        if isinstance(state, np.ndarray):
            if self.use_rnn:
                ...
                state_tensor = Tensor(state.reshape((n_env, -1)))
        ...
        if self.use_parameter_sharing:
            ...
            if self.use_rnn:
                ...
                actions_tensor = actions_tensor.reshape((n_env, 1, self.n_agents)).to(device_str)
            else:
                obs_input = {key: np.array([itemgetter(*self.agent_keys)(obs_dict)])}
                actions_tensor = actions_tensor.reshape((n_env, self.n_agents)).to(device_str)
            actions_onehot = {key: one_hot(actions_tensor.astype("int64"), self.action_space[key].n)}
        else:
            if self.use_rnn:
            ...
            actions_tensor = actions_tensor.to(device_str)
            actions_onehot = {k: one_hot(actions_tensor[:, i].astype("int64"), self.action_space[k].n)
        ...
        # values_out = values_out.gather(-1, actions_tensor.unsqueeze(-1).astype("int64"))
        # 在最后一个维度插入新维度并转换为 int64 类型
        actions_indices = paddle.unsqueeze(actions_tensor, axis=-1).astype("int64")  # 形状变为 (n_env, self.n_agents, 1)
        # 使用 paddle.gather_nd 提取对应值
        # 构造索引张量
        batch_size = values_out.shape[0]
        agent_dim = values_out.shape[1]
        ...
        values_selected = paddle.gather_nd(values_out, indices)  # 形状为 (n_env, self.n_agents)
        # 如果需要保持最后一个维度，可以添加新轴
        values_out = values_selected.unsqueeze(-1)  # 形状为 (n_env, self.n_agents, 1)
            ...
            for i in range(self.n_envs):
                if all(terminated_dict[i].values()) or truncated[i]:
                    ...
                    if self.use_wandb:
                        ...
                        step_info[f"Train-Results/Episode-Rewards/rank_{self.rank}/env-%d" % i] = info[i][
                            "episode_score"]
                    else:
