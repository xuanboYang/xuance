...
from xuance.paddlepaddle import LayerDict, REGISTRY_Representation, REGISTRY_Learners, Layer
from xuance.paddlepaddle.learners import learner
from xuance.paddlepaddle.utils import NormalizeFunctions, ActivationFunctions, init_distributed_mode
...
class MARLAgents(ABC):
    ...
    def __init__(self,
        ...
        if self.distributed_training and self.world_size > 1:
            if self.rank == 0:
                ...
                time_string_tensor = paddle.to_tensor(list(time_string.encode('utf-8')), dtype='uint8')
                if self.rank is not None:
                    time_string_tensor = time_string_tensor.cuda(self.rank)
            else:
                # time_string_tensor = torch.zeros(16, dtype=torch.uint8).to(self.rank)
                time_string_tensor = paddle.zeros([16], dtype='uint8')
                if self.rank is not None:
                    time_string_tensor = time_string_tensor.cuda(self.rank)
        ...
        self.policy: Optional[nn.Layer] = None
        ...
        if self.use_wandb:
            ...
            for k, v in info.items():
                ...
                for t in range(v.shape[0]):
                    frame = v[t]
                    if len(frame.shape) != 3 or frame.shape[0] not in [1, 3, 4]:
                        continue
                    frame = np.transpose(frame, (1, 2, 0))  # 转换为 (H, W, C)
                    self.writer.add_image('video_tag/frame', frame, step=t + 1 * v.shape[0])
    ...
    def _build_representation(self, representation_key: str,
        ...
        for key in self.model_keys:
            ...
            input_representations = dict(
                ...
                initialize=nn.initializer.Orthogonal,
    ...
    def _build_inputs(self,
        ...
        if self.use_parameter_sharing:
            ...
            agents_id = paddle.eye(self.n_agents).unsqueeze(0).expand([batch_size, self.n_agents, self.n_agents])
            if self.device is not None:
                # 修复cuda参数问题，提取数字设备ID或使用默认值0
                if 'gpu' in self.device:
                    ...
                    if ':' in self.device:
                        try:
                            ...
                            pass
                    agents_id = agents_id.cuda(device_id)
                else:
                    agents_id = agents_id.cpu()
